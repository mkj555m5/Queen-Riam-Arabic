...
## 🔗 الربط عبر الموقع (نظام جديد في 1.2.0-ar)

**لا يوجد ربط تلقائي نهائياً** — كل الربط يتم من موقع البوت المدمج:

1. شغّل البوت (محلياً أو على Railway) — الموقع يعمل معه على نفس المنفذ
2. افتح `http://localhost:3000/link` (أو رابط Railway)
3. اكتب رقم الواتساب الذي سيصبح هو البوت
4. سيظهر لك كود الربط المخصص: **RIAMBOOT**
5. في واتساب: الأجهزة المرتبطة ← ربط جهاز ← **الربط برقم الهاتف** ← أدخل الكود
6. فوراً يصلك على واتساب: «✅ البوت يعمل العادة» + **رمزك السري** (8 أحرف عشوائية)
7. ادخل `/login` بالرقم + الرمز السري → لوحة تحكم كاملة بإعدادات بوتك

> **حساب المالك:** الرقم في `OWNER_NUMBER` (افتراضي `201270221253`) عندما يربط ويدخل
> بالرمز السري يحصل تلقائياً على لوحة المالك: إدارة المستخدمين، ترقية العضويات المميزة 💎،
> حد عدد المسجلين (افتراضي 20)، وإعدادات الموقع.

---

## 🌐 لوحة تحكم الويب (مدمجة مع البوت)

| الصفحة | المسار | لمن؟ |
|--------|--------|------|
| الرئيسية + مميزات البوت | `/` | الجميع |
| ربط البوت (كود RIAMBOOT) | `/link` | الجميع |
| تسجيل الدخول (رقم + رمز سري) | `/login` | المسجلون |
| لوحة التحكم الكاملة | `/dashboard` | المسجلون + المالك |

**ماذا تتحكم به من اللوحة؟**
- إعدادات بوتك الخاصة لكل جلسة: القراءة التلقائية، الكتابة التلقائية، التسجيل الصوتي،
  متصل دائماً، تفاعل/رد الحالات، رفض المكالمات، وضع الأزرار، البادئة، نص الحالات — **تُطبق فوراً**
- **المالك إضافةً لذلك:** جدول كل المسجلين (ترقية 💎 / إرجاع / حذف / إعادة رمز سري)،
  حد المسجلين، فتح/غلق الربط، وإعلان الموقع

**العضوية المميزة 💎 في واتساب:**
- كل أوامر فئة `owner` (الإعدادات والسلوك: `.autoread` `.anticall` `.setprefix` `.mode`...) تصبح
  متاحة للأعضاء المميزين
- أوامر التحكم الكامل تبقى للمالك فقط: `.sudo` `.pair` `.reloadplugins` `.update` `.clearsession`
- فحص عضويتك من واتساب: `.premium`
...
النسخة الحالية مصممة لـ Railway — الموقع والبوت في خدمة واحدة:

1. ارفع المشروع على GitHub ثم في Railway: **New Project → Deploy from GitHub repo**
2. Railway يكتشف `railway.json` تلقائياً — أمر التشغيل `node index.js`
3. **مهم جداً:** أنشئ **Volume** واربطه بمسار **`/data`** (وليس `/app/data`!)
   - هذا المسار يحفظ الجلسات وبيانات المستخدمين بعد إعادة النشر
   - يمكن تغييره عبر متغير البيئة `DATA_DIR` إن أردت مساراً آخر
4. المتغيرات (Variables): ليس مطلوب شيء إلزامي — `PORT` يأتي تلقائياً
   - اختياري: `OWNER_NUMBER` (رقم المالك)، `BOT_NAME`، `PAIRING_CODE` (افتراضي RIAMBOOT)، مفاتيح API
5. بعد النشر افتح رابط Railway → سترى الموقع → `/link` لربط بوتك بكود **RIAMBOOT**

> ⚠️ بدون Volume على `/data` ستفقد الجلسات مع كل نشر جديد وستحتاج إعادة ربط جميع الأرقام.
>
> 🛡️ **عزل العمليات (v1.3):** كل جلسة واتساب تعمل في عملية Node.js مستقلة —
> لو علق بوت مستخدم (مثلاً فيديو ضخم) أو انهار، بقية البوتات والموقع تعمل طبيعياً،
> والنظام يعيد تشغيل الجلسة المتضررة تلقائياً.
...
## 🆕 ما الجديد في 1.2.0-ar (إصدار Railway)

| الميزة | الشرح |
|--------|-------|
| 🚫 إزالة الربط التلقائي | لم يعد البوت يطلب رمز ربط من الطرفية — كل الربط من الموقع |
| 🌐 موقع متكامل | رئيسية بالمميزات + ربط + دخول + لوحة تحكم — بثيم Queen Riam النيون |
| 🔑 كود ربط مخصص | **RIAMBOOT** — كود ثابت أنيق لكل عمليات الربط (مع fallback تلقائي) |
| 🔐 رمز سري لكل رقم | يُولد عشوائياً ويُرسل على واتساب بعد الربط — هو كلمة سر لوحة التحكم |
| 👑 حساب المالك | `201270221253` يرى المستخدمين، يرقّي العضويات، يضبط حد التسجيل (20 افتراضياً) |
| 💎 العضوية المميزة | أوامر المالك العادية تُتاح للمميزين — وأوامر التحكم الكامل تبقى للمالك |
| 🚄 جاهز لـ Railway | `railway.json` + فحص صحة `/api/status` + منفذ واحد للبوت والموقع |
