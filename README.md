...
3. **مهم جداً:** أنشئ **Volume** واربطه بمسار `/app/data` (لحفظ الجلسات والبيانات بعد إعادة النشر)
   - أو أنشئ Volume على `/app` بالكامل إن أردت حفظ كل شيء
4. المتغيرات (Variables): ليس مطلوب شيء إلزامي — `PORT` يأتي تلقائياً
   - اختياري: `OWNER_NUMBER` (رقم المالك)، `BOT_NAME`، مفاتيح API
5. بعد النشر افتح رابط Railway → سترى الموقع → `/link` لربط بوتك بكود **RIAMBOOT**

> ⚠️ بدون Volume سيُحذف مجلد `data/` مع كل نشر جديد وستحتاج إعادة ربط جميع الأرقام.
