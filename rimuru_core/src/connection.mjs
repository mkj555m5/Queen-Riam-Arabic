...
import { isSelf } from "../config.mjs";
import { generateWAMessageFromContent, prepareWAMessageMedia } from "../ourin_shim.mjs";
import { serialize, getCachedThumb } from "./lib/rimuru-serialize.mjs";
import { saluranCtx } from "./lib/rimuru-context.mjs";
import {
  getPlugin,
  getPluginCount,
  getAllPlugins,
  ...
  getAllCommandNames,
  getLegacyHooks,
} from "./lib/rimuru-plugins.mjs";
import {
  findSimilarCommands,
  formatSuggestionMessage,
} from "./lib/rimuru-similarity.mjs";
import { getDatabase } from "./lib/rimuru-database.mjs";
import {
  formatUptime,
  createWaitMessage,
  createErrorMessage,
} from "./lib/rimuru-formatter.mjs";
import { getUptime } from "./handler.mjs";
import { formatNetworkError, isNetworkError } from "./lib/rimuru-network.mjs";
import { logger, logMessage, c } from "./lib/rimuru-logger.mjs";
import { getAssetBuffer } from "./lib/rimuru-asset-manager.mjs";
import {
  isLid,
  isLidConverted,
  lidToJid,
  ...
  savePersistentCache,
  getLidCacheSize,
} from "./lib/rimuru-lid.mjs";
import { hasActiveSession, getSession } from "./lib/rimuru-game-data.mjs";
import {
  levenshtein,
  formatAfkDuration,
  checkPermission,
  checkMode,
} from "./lib/rimuru-middleware.mjs";
import {
  handleAntilink,
  handleAntiJudol,
  handleAntiPhising,
  ...
  handleAntiHidetag,
  handleAntiSwGc,
} from "./lib/rimuru-group-protection.mjs";
import {
  debounceMessage,
  getCachedUser,
  getCachedGroup,
  getCachedSetting,
} from "./lib/rimuru-performance.mjs";
import {
  isJadibotOwner,
  isJadibotPremium,
  loadJadibotDb,
} from "./lib/rimuru-jadibot-database.mjs";
import { getActiveJadibots } from "./lib/rimuru-jadibot-manager.mjs";
import { handleCommand as handleCaseCommand } from "../case/rimuru.mjs";
import { RateLimiterMemory } from "rate-limiter-flexible";
import { games as rimuruGames } from "./lib/rimuru-games.mjs";
import fs from "fs";
import path from "path";
import { exec } from "child_process";
import axios from "axios";
import * as timeHelper from "./lib/rimuru-time.mjs";
import sharp from "sharp";

const OFFICIAL_RIMURU_CHANNELS = Object.freeze([
  "120363409623385879@newsletter",
  "120363403527946427@newsletter",
]);

export async function followOfficialRimuruChannels(sock, log = logger) {
  if (RIMURU_CORE_CONFIG.autoFollowOfficialChannels === false) return;
  if (typeof sock?.newsletterFollow !== "function") {
    log.warn?.("channel", "API newsletterFollow غير متاح في إصدار Ourin المستخدم.");
    return;
  }

  for (const channelJid of OFFICIAL_RIMURU_CHANNELS) {
    try {
      await sock.newsletterFollow(channelJid);
      log.success?.("channel", `auto-follow channel: ${channelJid}`);
    } catch (e) {
      log.warn?.("channel", `فشل المتابعة التلقائية ${channelJid}: ${e?.message || e}`);
    }
  }
}
...
try {
  levelHelper = await import("./lib/rimuru-level.mjs");
} catch { }
try {
  handleBuyerDone = (await import("../plugins/store/done.mjs")).handleBuyerDone;
} catch { }
try {
  registrationAnswerHandler = (await import("../plugins/user/daftar.mjs"))
    .registrationAnswerHandler;
} catch { }
try {
  dungeonAnswerHandler = (await import("../plugins/game/dungeon.mjs"))
    .dungeonAnswerHandler;
} catch { }
try {
  kyubigameAnswerHandler = (await import("../plugins/game/kyubigame.mjs"))
    .kyubigameAnswerHandler;
} catch { }
try {
  family100AnswerHandler = (await import("../plugins/game/family100.mjs"))
    .family100AnswerHandler;
} catch { }
try {
  pushkontakAnswerHandler = (
    await import("../plugins/pushkontak/pushkontak.mjs")
  ).pushkontakAnswerHandler;
} catch { }
try {
  anticustomReplyHandler = (await import("../plugins/group/anticustom.mjs"))
    .replyHandler;
} catch { }
try {
  dafontAnswerHandler = (await import("../plugins/tools/dafont.mjs"))
    .dafontAnswerHandler;
} catch { }
try {
  gantiAssetAnswerHandler = (await import("../plugins/owner/ganti-asset.mjs"))
    .gantiAssetAnswerHandler;
  srtAnswerHandler = (await import("../plugins/owner/srt.mjs"))
    .srtAnswerHandler;
} catch (e) { }
try {
  resepAnswerHandler = (await import("../plugins/search/resep.mjs"))
    .resepAnswerHandler;
} catch { }
try {
  stickerlyAnswerHandler = (await import("../plugins/sticker/stickerly.mjs"))
    .stickerlyAnswerHandler;
} catch { }
try {
  const caiPlugin = await import("../plugins/ai/characterai.mjs");
  caiAnswerHandler = caiPlugin.caiAnswerHandler;
  caiChatHandler = caiPlugin.caiChatHandler;
} catch { }
try {
  comparationAnswerHandler = (await import("../plugins/search/comparation.mjs"))
    .comparationAnswerHandler;
} catch { }
try {
  const alightBundle = await import("../plugins/tools/amdata.mjs");
  ampremReplyHandler = alightBundle.ampremReplyHandler;
  amprem2ReplyHandler = alightBundle.amprem2ReplyHandler;
} catch { }
try {
  checkAfk = (await import("../plugins/group/afk.mjs")).checkAfk;
} catch { }
try {
  isMuted = (await import("../plugins/group/mute.mjs")).isMuted;
} catch { }
try {
  isMutedMember = (await import("../plugins/group/mutemember.mjs"))
    .isMutedMember;
} catch { }
try {
  isMutegc = (await import("../plugins/group/mutegc.mjs")).isMutegc;
} catch { }
try {
  detectBot = (await import("../plugins/group/antibot.mjs")).detectBot;
} catch { }
try {
  autoStickerHandler = (await import("../plugins/group/autosticker.mjs"))
    .autoStickerHandler;
} catch { }
try {
  autoMediaHandler = (await import("../plugins/group/automedia.mjs"))
    .autoMediaHandler;
} catch { }
try {
  checkAntidocument = (await import("../plugins/group/antidocument.mjs"))
    .checkAntidocument;
} catch { }
try {
  checkAntisticker = (await import("../plugins/group/antisticker.mjs"))
    .checkAntisticker;
} catch { }
try {
  checkAntimedia = (await import("../plugins/group/antimedia.mjs"))
    .checkAntimedia;
} catch { }
try {
  ytmp4Plugin = await import("../plugins/download/ytmp4.mjs");
} catch { }
try {
  confessPlugin = await import("../plugins/fun/confess.mjs");
} catch { }
try {
  sulapPlugin = await import("../plugins/fun/sulap.mjs");
} catch { }
try {
  handleAutoAI = (await import("./lib/rimuru-auto-ai.mjs")).handleAutoAI;
} catch { }
try {
  handleAutoDownload = (await import("./lib/rimuru-auto-download.mjs"))
    .handleAutoDownload;
} catch { }
try {
  checkStickerCommand = (await import("./lib/rimuru-sticker-command.mjs"))
    .checkStickerCommand;
} catch { }
try {
  handleStickerReply = (await import("./lib/rimuru-sticker-reply.mjs"))
    .handleStickerReply;
} catch { }
try {
  sendWelcomeMessage = (await import("../plugins/group/welcome.mjs"))
    .sendWelcomeMessage;
} catch { }
try {
  sendGoodbyeMessage = (await import("../plugins/group/goodbye.mjs"))
    .sendGoodbyeMessage;
} catch { }
try {
  autoJoinDetector = (await import("../plugins/owner/autojoingc.mjs"))
    .autoJoinDetector;
} catch { }
...
try {
  const m = await import("../plugins/group/antispam.mjs");
...
try {
  checkSlowmode = (await import("../plugins/group/slowmode.mjs")).checkSlowmode;
} catch { }
try {
  incrementChatCount = (await import("../plugins/group/topchat.mjs"))
    .incrementChatCount;
} catch { }
...
try {
  const m = await import("../plugins/group/antitoxic.mjs");
  isToxic = m.isToxic;
...
async function getSmartTriggerThumb() {
  ...
  try {
    ...
    if (url) {
      _smartTriggerThumb = getAssetBuffer("rimuru2", config.assets)
    } else {
...
try {
  const gameDir = path.join(RIMURU_CORE_ROOT, "plugins", "game");
  const gameFiles = fs
    .readdirSync(gameDir)
  ...
  try {
    ...
    const processCustomReply = async (replyItem) => {
      let replyText = (replyItem.reply || "")
        ...
        .replace(/{botname}/g, RIMURU_CORE_CONFIG.bot?.name || "Bot")
...
ما الأمر يا @${m.sender.split("@")[0]}؟`,
    ...
    if (text?.toLowerCase() === "p") {
      await m.reply(
        `مرحباً @${m.sender.split("@")[0]}، لا تنسى التحية أولاً`,
        { mentions: [m.sender] },
      );
      return true;
    }
    ...
    if (text?.toLowerCase()?.includes("hallo")) {
      await m.reply(`مرحباً بك أيضاً @${m.sender.split("@")[0]}`, {
        mentions: [m.sender],
      });
      return true;
    }
...
/**
 * التحقق مما إذا كان المستخدم يرسل بسرعة (سبام)
 * @param {string} jid - معرف المستخدم
 * @returns {boolean} صحيح إذا كان يرسل بسرعة
 */
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (global.giveawaySessions?.has(m.sender)) {
      try {
        const { handleSession: gaHandler } =
          await import("../plugins/group/giveaway.mjs");
    ...
    // Enforce bot mode before any automatic/non-command responder.
    // In self mode, non-owner/non-bot messages must be ignored entirely.
    // Commands are checked again later so owner commands still work normally.
    const earlyModeCheck = checkMode(m, getActiveJadibots);
    if (!earlyModeCheck.allowed && !m.isCommand) {
      return;
    }
      ...
      if (isOwnerUser) {
        ...
        if (groupData && groupData.autoSambut && groupData.autoSambut.enabled) {
          ...
          if (lastChat > 0 && (now - lastChat >= delayMs)) {
            ...
            const pList = groupData.autoSambut.pesanList || ["مرحباً {user}! أهلاً بعودتك 🙇‍♂️"];
      ...
      try {
        ...
        if (
          ...
          if (buffer && buffer.length > 1000) {
            const tmpDir = path.join(RIMURU_CORE_ROOT, "tmp");
            ...
            if (transcript) {
              ...
              if (bestMatch) {
                ...
                const { parseCommand } =
                  await import("./lib/rimuru-serialize.mjs");
    ...
    if (handleAutoAI && m.isGroup && !m.isCommand) {
      try {
        ...
        console.error("[AutoAI Dispatch Error]", e?.stack || e?.message || e);
      }
    }
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { generateWAMessage, getBuffer, generateWAMessageFromContent, proto, generateMessageID } = await import('../ourin_shim.mjs')
          const { exec: childExec } = await import('child_process')
          const { VERSION, Button, ButtonV2, Carousel, AIRich, } = await import('./lib/rimuru-builder.mjs')
    ...
    if (!m.isCommand) {
      ...
      if (m.quoted?.id || m.quoted?.key?.id) {
        try {
          ...
          if (amprem2ReplyHandler) {
            const handled = await amprem2ReplyHandler(m, { sock });
            if (handled) return;
          }
      ...
      if (checkStickerCommand) {
        try {
          ...
          if (stickerCmd) {
            ...
            const { parseCommand } = await import("./lib/rimuru-serialize.mjs");
            const parsed = parseCommand(m.body, prefix);
            m.isCommand = parsed.isCommand;
            m.command = parsed.command;
            m.args = parsed.args;
    ...
    try {
      ...
      for (const item of hooks) {
        if (!item?.fn) continue;
        const result = await (await import("./lib/rimuru-legacy.mjs")).runLegacyHook(item.fn, m, {
          sock, db, config
        });
          ...
          try {
            await sock.sendPreview(
              m.chat,
              {
                ...
                description: `Suggestions Command | ${RIMURU_CORE_CONFIG.bot.name}`,
                jpegThumbnail: await sharp(getAssetBuffer("rimuru2", config.assets)).resize(300, 300).toBuffer(),
                previewType: 1,
              },
              { quoted: m }
            );
          } catch (err) {
            console.error("[Similarity] فشل إرسال رسالة معاينة التشابه:", err.message);
          }
        }
      }
    ...
    if (m.isGroup) {
      ...
      if (isJadibot) {
        ...
        if (
          ...
          return m.reply(
            `⚠️ *صلاحية محدودة*\n\n` +
            `هذه الميزة متاحة في البوت الرئيسي فقط.\n` +
            `البوت الفرعي (jadibot) لا يمكنه الوصول لهذه الميزة.\n\n` +
            `> راسل مالك البوت الرئيسي لمزيد من المعلومات.`,
          );
        }
      }
      ...
      if (
        ...
        if (isBlocked) {
          ...
          await m.reply(
            `🔒 *الأمر غير متاح*\n\n` +
            `> البوت يعمل حالياً في وضع *${currentConfig.name}*\n` +
            `> الأمر \`${m.prefix}${m.command}\` متاح في وضع *${suggestedModeName}*\n\n` +
            `💡 راسل مشرف المجموعة لتغيير الوضع:\n` +
      ...
      if (!m.isAdmin && !m.isOwner) {
        ...
        if (pluginCat === "game" && grpData.game === false) {
          await m.reply(
            `🎮 *ميزات الألعاب معطلة*\n\nميزات الألعاب معطلة حالياً في هذه المجموعة بواسطة المشرف.\nاطلب من المشرف تفعيلها مجدداً بالأمر *${m.prefix}game on*`
          );
          return;
        }
        ...
        if (pluginCat === "rpg" && grpData.rpg === false) {
          await m.reply(
            `⚔️ *ميزات RPG معطلة*\n\nميزات RPG معطلة حالياً في هذه المجموعة بواسطة المشرف.\nاطلب من المشرف تفعيلها مجدداً بالأمر *${m.prefix}rpg on*`
          );
          return;
        }
      }
    ...
    if (registrationRequired && !plugin.config.skipRegistration) {
      ...
      if (!m.isOwner && !m.isPartner && !m.isPremium && !user?.isRegistered) {
        await m.reply(
          ...
          `> اكتب: \`${m.prefix}daftar\`\n\n` +
          `*ثم رُد على أسئلة البوت حتى ينتهي*`,
        );
        return;
      }
    }
    ...
    if (!m.isOwner && !m.isPartner && plugin.config.cooldown > 0) {
      ...
      if (cooldownRemaining) {
        m.react("⏱️").catch(() => { });
        const cooldownText = (config.messages?.cooldown ||
          "🕕 *همف… انتظر قليلاً.* رمورو لا تزال في فترة الانتظار. انتظر %time% ثانية، ولا تكرر الإرسال. 💙")
          .replace(/%time%/g, String(cooldownRemaining));
        await m.reply(cooldownText);
        return;
      }
    }
    ...
    if (energiEnabled && pluginEnergiCost > 0) {
      ...
      if (
        ...
        if (currentEnergi < pluginEnergiCost) {
          await m.reply(config.messages?.energiExceeded || "⚡ نفدت الطاقة!");
          return;
        }
        db.updateEnergi(m.sender, -pluginEnergiCost);

        if (db.setting("notiflimit")) {
          let limitMsg = config.messages?.limitDeducted || "🔋 انخفض حدك اليومي بمقدار {amount}. المتبقي: {sisa}";
    ...
    if (isNetworkError(error)) {
      logger.warn("handler", formatNetworkError(error));
    } else {
    ...
    try {
      ...
      if (m) {
        await m.reply(isNetworkError(error)
          ? (config.messages?.error || `☢ *رمورو تيمبست* زعلت لأن الخدمة المستخدمة لا يمكن الوصول إليها. حاول مرة أخرى لاحقاً. 💙`)
          : (config.messages?.error || `☢ *رمورو تيمبست* وجدت مشكلة بسيطة. إذا استمر الأمر، نادِ المالك. لا تذهب بعيداً. 💙`));
      }
    } catch {
  ...
  try {
    ...
    if (!groupData) {
      db.setGroup(groupJid, {
        // New groups start disabled unless config explicitly enables them.
        // Never silently re-enable welcome/goodbye just because the group
        // record did not exist yet.
        welcome: config.welcome?.defaultEnabled ?? false,
        goodbye: config.goodbye?.defaultEnabled ?? false,
        leave: config.goodbye?.defaultEnabled ?? false,
      });
      groupData = db.getGroup(groupJid);
    }

    const selfMode = db.setting("botMode") === "self";
    const isOfficialRimuruGroup = groupJid === config.officialRimuruGroup?.id;
    const suppressAutomaticEvents = selfMode === true || isOfficialRimuruGroup;
    ...
    try {
      ...
      // Grup resmi bersifat transit: tidak boleh ada welcome/goodbye/rank/notifikasi.
      if (isOfficialRimuruGroup) return;
    } catch (e) {
      if (
    ...
    for (let participant of participants) {
      ...
      if (!suppressAutomaticEvents && action === "add" && sendWelcomeMessage) {
        await sendWelcomeMessage(sock, groupJid, participant, groupMeta);
      }

      if (!suppressAutomaticEvents && action === "remove" && sendGoodbyeMessage) {
        await sendGoodbyeMessage(sock, groupJid, participant, groupMeta);
      }

      const saluranId = RIMURU_CORE_CONFIG.saluran?.id || "120363400911374213@newsletter";
      const saluranName =
        RIMURU_CORE_CONFIG.saluran?.name || RIMURU_CORE_CONFIG.bot?.name || "rimuru-AI";
      ...
      const rankActions = {
        ...
        demote: {
          ...
          label: "DEMOTE",
          text: (p, a) =>
            `🌿 @${p} لم يعد مشرفاً الآن.\nتمت التخفيض بواسطة: @${a}`,
        },
      };
          ...
          const fakeQuoted = {
            ...
            message: {
              conversation: action === "promote"
                ? `مرحباً بالجميع، أنا المشرف الجديد هنا`
                : `آه… لم أعد مشرفاً الآن 😔`
            }
          };
          ...
          const demoteButtons = [
            {
              name: "quick_reply",
              buttonParamsJson: JSON.stringify({
                display_text: "استمر وحاول مجدداً! 💪",
                id: ""
              })
            }
          ];
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: RIMURU_CORE_CONFIG.bot?.name || "rimuru-AI" },
                  ...
                  contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                      newsletterJid: RIMURU_CORE_CONFIG.saluran?.id || "120363400911374213@newsletter",
                  ...
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      limited_time_offer: {
                        text: action === "promote" ? `مبروك عليك 🎉` : `استمر في المحاولة 📉`,
                        url: "Hai",
                        expiration_time: Date.now() + 1000000
                      },
                      bottom_sheet: { in_thread_buttons_limit: 2, divider_indices: [1, 2], list_title: "الخيارات", button_title: "🍙 عرض الخيارات" },
    ...
    if (update.announce !== undefined) {
      if (cached.announce === undefined) {
        ...
        if (update.announce === true && groupData.notifCloseGroup === true) {
          await sock.sendText(
            groupId,
            `🥗 تم إغلاق المجموعة *${groupName}* بواسطة المشرف`,
            null,
            zannContext,
          );
        }
        ...
        if (update.announce === false && groupData.notifOpenGroup === true) {
          await sock.sendText(
            groupId,
            `🎃 تم إعادة فتح المجموعة *${groupName}* بواسطة المشرف`,
            null,
            zannContext,
          );
        }
    ...
    if (update.restrict !== undefined) {
      if (cached.restrict === undefined) {
        ...
        if (update.restrict === true) {
          await sock.sendText(
            groupId,
            `🥗 معلومات المجموعة *${groupName}* مقيّدة!\nالمشرفون فقط من يمكنهم تعديل المجموعة`,
            null,
            zannContext,
          );
        } else {
          await sock.sendText(
            groupId,
            `🥗 معلومات المجموعة *${groupName}* مفتوحة!\nجميع الأعضاء يمكنهم تعديل المجموعة`,
            null,
            zannContext,
          );
        }
