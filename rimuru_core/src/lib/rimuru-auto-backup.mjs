...
function shouldExclude(filePath) {
  const relativePath = path.relative(RIMURU_CORE_ROOT, filePath);
...
async function createBackup() {
  return new Promise((resolve, reject) => {
    ...
    const rootDir = RIMURU_CORE_ROOT;
...
async function sendBackupToOwner(backupInfo) {
  ...
  try {
    ...
    const caption =
      ...
      `┃ ⏱️ الفاصل الزمني: ${formatInterval(state.intervalMs)}\n` +
      `┃ #️⃣ النسخة الاحتياطية رقم ${state.backupCount + 1}\n` +
      `╰┈┈┈┈┈┈┈┈⬡\n\n` +
      `> ${RIMURU_CORE_CONFIG.bot?.name || "rimuru-AI"} Auto Backup System`;
...
function enableAutoBackup(intervalStr, sock) {
  ...
  if (!parsed) {
    return {
      success: false,
      error: "صيغة الفاصل الزمني غير صالحة. مثال: 5m، 1h، 2d",
    };
  }
