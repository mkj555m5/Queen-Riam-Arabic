...
import { downloadMediaMessage, getContentType } from "../../ourin_shim.mjs";
import { addExifToWebp, DEFAULT_METADATA } from "./rimuru-exif.mjs";
...
import {
  getProfilePicture,
  getProfileBuffer,
} from "./rimuru-profile-picture.mjs";
...
async function extendSocket(sock) {
  ...
  sock.sendMedia = async function (
    ...
    if (Buffer.isBuffer(source)) {
      ...
      throw new Error(
        "يجب أن يكون المصدر Buffer أو رابط URL أو مسار ملف أو كائن { url } أو حمولة وسائط",
      );
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = RIMURU_CORE_CONFIG.bot?.name || "rimuru-AI";
    if (source) {
      let data = source;
