...
async function uploadTo0x0(buffer, opts) {
  if (!Buffer.isBuffer(buffer)) throw new Error("يجب أن يكون buffer من نوع Buffer");
  ...
  if (res.status < 200 || res.status >= 300) {
    throw new Error(`فشل الرفع (HTTP ${res.status}): ${typeof res.data === "string" ? res.data : JSON.stringify(res.data)}`);
  }
  ...
  throw new Error("لا توجد بيانات صالحة في استجابة Termai");
}
