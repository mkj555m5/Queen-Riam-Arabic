...
class Database {
  ...
  migrateFromOldPath() {
    const oldPath = path.join(RIMURU_CORE_ROOT, "src", "database");
    ...
    logger.info(
      "database",
      `migrasi ${oldFiles.length} file dari src/database ke ${path.relative(RIMURU_CORE_ROOT, this.dbPath)}/`,
    );
    this.ensureDir();
    ...
    for (const file of oldFiles) {
      ...
      try {
        ...
        logger.error("database", `فشل ترحيل ${file}: ${e.message}`);
      }
    }
    logger.success("database", "اكتمل ترحيل المسارات");
  }
  ...
  async init() {
    try {
      ...
      const fileMap = {
        ...
        autoai: { file: "autoai.json", defaults: defaultAutoAI },
      };
      ...
      this.db.data = {
        ...
        autoai: this.stores.autoai.data,
      };
      ...
      if (lastDefault !== currentDefault || lastPremium !== currentPremium) {
        ...
        if (synced > 0) {
          logger.info(
            "database",
            `تمت مزامنة طاقة ${synced} مستخدماً (الافتراضي: ${currentDefault}، بريميوم: ${currentPremium})`,
          );
        }
      }
      ...
      logger.success(
        "database",
        "قاعدة البيانات جاهزة (حفظ تلقائي كل 5 ثوانٍ)",
      );
      return this;
    } catch (error) {
      logger.error("database", `فشل التهيئة: ${error.message}`);
      this.db = {
        data: {
          users: {},
          groups: {},
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (!content || content === "" || content === "{}") {
        ...
        try {
          ...
          logger.warn(
            "database",
            `${fileName} تالف، حُفظت نسخة احتياطية: ${backup}`,
          );
        }
      }
    } else {
  ...
  async migrateFromSingleFile() {
    ...
    try {
      ...
      logger.success(
        "database",
        `اكتمل الترحيل من energis.json، النسخة الاحتياطية: ${path.basename(backupPath)}`,
      );
    } catch (e) {
      logger.error("database", `فشل ترحيل energis.json: ${e.message}`);
    }
  }
  ...
  async save() {
    try {
      ...
      logger.error("database", `فشل الحفظ: ${error.message}`);
      return false;
    }
  }
...
function getDatabase() {
  if (!dbInstance) {
    throw new Error(
      "قاعدة البيانات غير مهيأة. استدعِ initDatabase أولاً.",
    );
  }
  return dbInstance;
}
