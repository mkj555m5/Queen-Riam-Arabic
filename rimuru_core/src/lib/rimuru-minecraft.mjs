import {
  ...
  CRAFT_RECIPES,
  JACKPOT_POOLS,
} from "./rimuru-minecraft-data.mjs";
...
function addPickExp(mcUser, pickKey, amount) {
  ...
  while (pick.exp >= pick.expToNextLevel && pick.level < pick.maxLevel) {
    ...
    levelUp = true;
    statsIncreased.push("الحظ +1%، السرعة +1%، الفورتشن +0.5%");
  }
  if (levelUp)
    return `⛏️ ارتقى الفأس *${pick.name}* إلى المستوى ${pick.level}!\n✨ ${statsIncreased.join(", ")}`;
  return null;
}
...
function doCombat(mcUser, mobKey) {
  ...
  if ((mcUser.level || 1) < mobDef.minLevel)
    return { error: `⬆️ مستواك غير كافٍ! يلزم Lv.${mobDef.minLevel}` };
  ...
  while (pHp > 0 && mHp > 0) {
    ...
    mHp -= pDmg;
    log.push(`⚔️ أنت: -${pDmg} صحة على ${mobDef.name}`);
    if (mHp <= 0) break;
    const mDmg = Math.max(
      1,
    ...
    pHp -= mDmg;
    log.push(`💥 ${mobDef.name}: -${mDmg} صحة عليك`);
  }
...
function doCraft(mcUser, recipeKey) {
  ...
  if (!recipe) return { error: "🛠️ الوصفة غير موجودة!" };
  if ((mcUser.level || 1) < recipe.requiredLevel)
    return { error: `⬆️ مستواك غير كافٍ! يلزم Lv.${recipe.requiredLevel}` };
  ...
  for (const [ingName, count] of Object.entries(recipe.ingredients)) {
    ...
    if (found < count)
      return {
        error: `🧪 المواد ناقصة! تحتاج ${count} من ${ingName} ولديك ${found}`,
      };
  }
...
function applyJackpotReward(db, mcUser, sender, result) {
  ...
  switch (type) {
    case "coins":
      ...
      applied = { desc: `💰 عملة +${formatMoney(val)}`, type };
      break;
    case "energi": {
      const cleanJid = sender.split("@")[0];
      const user = db.db.data.users[cleanJid];
    ...
    case "limit": {
      ...
      applied = { desc: `📌 الحد اليومي +${val}`, type };
      break;
    }
    case "tickets":
      f.gachaTickets = (f.gachaTickets || 0) + val;
      applied = { desc: `🎟️ تذاكر غاتشا +${val}`, type };
      break;
    case "tokens":
      f.prestigeTokens = (f.prestigeTokens || 0) + val;
      applied = { desc: `🪙 رموز برستيج +${val}`, type };
      break;
    case "exp_boost": {
      const boost = Math.floor(f.expToNextLevel * val * 0.5);
      f.exp = (f.exp || 0) + boost;
      applied = { desc: `⭐ خبرة +${formatMoney(boost)} (تعزيز x${val})`, type };
      break;
    }
    case "premium_7d":
    case "premium_30d": {
      ...
      applied = { desc: `👑 بريميوم ${days} يوم!`, type };
      break;
    }
    case "partner_7d":
    case "partner_30d": {
      ...
      applied = { desc: `🤝 شراكة ${days} يوم!`, type };
      break;
    }
    case "unlimited_energi": {
      ...
      applied = { desc: `♾️ طاقة غير محدودة!`, type };
      break;
    }
    case "unlimited_limit": {
      ...
      applied = { desc: `♾️ حد يومي غير محدود!`, type };
      break;
    }
    case "owner_reward": {
      const cleanJid = sender.split("@")[0];
      ...
      applied = {
        desc: `🔥 جائزة المالك! بريميوم + شراكة لمدة سنة كاملة + غير محدود!`,
        type,
      };
      break;
    }
    default:
      applied = { desc: `🎁 جائزة غامضة!`, type: "mystery" };
  }
  return applied;
}
...
function healPlayer(mcUser) {
  ...
  if (mcUser.hp >= mcUser.maxHp) return { error: "❤️ صحتك ممتلئة بالفعل!" };
  if ((mcUser.money || 0) < cost)
    return { error: `💸 المال غير كافٍ! يلزم ${formatMoney(cost)}` };
  mcUser.money -= cost;
  mcUser.hp = mcUser.maxHp;
