...
/**
 * رسم لوحة اللعبة مع مواقع اللاعبين
    ...
    try {
        ...
        for (const player of playerPositions) {
            ...
            try {
                ...
                console.log(`[ULARTANGGA] فشل تحميل صورة اللاعب ${player.index}:`, e.message);
            }
        }

        return await canvas.encode('png');
    } catch (error) {
        console.error('[ULARTANGGA] خطأ في رسم اللوحة:', error.message);
        return null;
    }
}

/**
 * جلب إعدادات خريطة لوحة عشوائية
