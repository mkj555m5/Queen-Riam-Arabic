...
const DB_PATH = path.join(RIMURU_CORE_ROOT, 'src', 'database')
...
function addOwner(jid, name = 'Unknown') {
    const cleanJid = jid?.replace(/@.+/g, '') || ''
    if (isOwner(cleanJid)) return { success: false, message: 'مالك بالفعل' }
    ...
    saveOwners(owners)
    return { success: true, message: 'أُضيف بنجاح كمالك' }
}
...
function removeOwner(jid) {
    ...
    if (index === -1) return { success: false, message: 'غير موجود في قائمة المالكين' }

    owners.splice(index, 1)
    saveOwners(owners)
    return { success: true, message: 'حُذف من المالكين بنجاح' }
}
...
function addPremium(jid, days = 30, name = 'Unknown') {
    ...
    if (existing !== -1) {
        ...
        return { success: true, message: `تم تمديد البريميوم ${days} يوم`, expiredAt: premiumList[existing].expiredAt }
    }
    ...
    savePremium(premiumList)
    return { success: true, message: `أُضيف بنجاح كبريميوم لمدة ${days} يوم`, expiredAt }
}
...
function removePremium(jid) {
    ...
    if (index === -1) return { success: false, message: 'غير موجود في قائمة البريميوم' }

    premiumList.splice(index, 1)
    savePremium(premiumList)
    return { success: true, message: 'حُذف من البريميوم بنجاح' }
}
...
function addPartner(jid, days = 30, name = 'Unknown') {
    ...
    if (existing !== -1) {
        ...
        return { success: true, message: `تم تمديد الشراكة ${days} يوم`, expiredAt: partnerList[existing].expiredAt }
    }
    ...
    savePartners(partnerList)
    return { success: true, message: `أُضيف بنجاح كشريك لمدة ${days} يوم`, expiredAt }
}
...
function removePartner(jid) {
    ...
    if (index === -1) return { success: false, message: 'غير موجود في قائمة الشركاء' }

    partnerList.splice(index, 1)
    savePartners(partnerList)
    return { success: true, message: 'حُذف من الشركاء بنجاح' }
}
