...
function getApiKey() {
    ...
    if (!key) throw new Error('مفتاح JasaOTP API غير مُعد في config.js')
    return key
}
...
async function getBalance() {
    ...
    if (!data.success) throw new Error(data.message || 'فشل فحص الرصيد')
...
async function getCountries() {
    ...
    if (!data.success) throw new Error(data.message || 'فشل جلب قائمة الدول')
...
async function getServices(countryId) {
    ...
    if (Object.keys(services).length === 0) {
        throw new Error('لا توجد خدمات متاحة لهذه الدولة')
    }
...
async function getOperators(countryId) {
    ...
    if (!data.success) throw new Error(data.message || 'فشل جلب قائمة المشغّلين')
...
async function createOrder(countryId, service, operator) {
    ...
    if (!data.success) throw new Error(data.message || 'فشل إنشاء طلب OTP')
...
async function cancelOrder(orderId) {
    ...
    if (!data.success) throw new Error(data.message || 'فشل إلغاء الطلب')
