import { RIMURU_CORE_ROOT } from "../../rimuru_paths.mjs";
import { RIMURU_CORE_CONFIG } from "../../rimuru.mjs";
import { exec } from "child_process";
import { promisify } from "util";
import { logicBell, transcribeAudio } from "../scraper/termai.mjs";
import { getDatabase } from "./rimuru-database.mjs";
import { pinterest } from "btch-downloader";
import config from "../../config.mjs";
import axios from "axios";
import path from "path";
import fs from "fs";
const userCooldowns = new Map();
const COOLDOWN_MS = 3000;

const ACTION_REGEX = /\[ACTION\s*:\s*(\w+)(?:\s+([^\]]*))?\]/gi;

const SYSTEM_PROMPT_ACTIONS = `
لديك قدرة خاصة على تنفيذ الأكشن وإرسال الرسائل الغنية (RICH MESSAGE) في واتساب.
إذا طلب المستخدم شيئاً يطابق أحد الأكشن أدناه، أدرِج وسم الأكشن في نهاية رسالتك.
إذا كان سياق إجابتك يطابق رسالة غنية، استخدم صيغة الرسالة الغنية أدناه.

صيغة الأكشن (ضعها في نهاية الرسالة، ويمكن أن تكون أكثر من واحدة):
[ACTION:KICK target=628xxx@s.whatsapp.net]
[ACTION:ADD target=628xxx]
[ACTION:PROMOTE target=628xxx@s.whatsapp.net]
[ACTION:DEMOTE target=628xxx@s.whatsapp.net]
[ACTION:LEAVE]
[ACTION:OPEN]
[ACTION:CLOSE]
[ACTION:TAGALL]
[ACTION:HIDETAG message=الرسالة التي تريد إرسالها]
[ACTION:SETNAME name=اسم المجموعة الجديد]
[ACTION:SETDESC desc=وصف المجموعة الجديد]
[ACTION:DELETE]
[ACTION:WARN target=628xxx@s.whatsapp.net]
[ACTION:STICKER]
[ACTION:ANTILINK mode=on]
[ACTION:PINS query=كلمات البحث]

قائمة الأكشن:
- KICK: طرد عضو من المجموعة. يحتاج target.
- ADD: إضافة عضو إلى المجموعة. يحتاج رقم (628xxx).
- PROMOTE: ترقية عضو ليصبح مشرفاً. يحتاج target.
- DEMOTE: تخفيض مشرف ليصبح عضواً عادياً. يحتاج target.
- LEAVE: خروج البوت من هذه المجموعة. فقط إذا طلب المالك (owner).
- OPEN: فتح المجموعة ليتسنى لجميع الأعضاء الدردشة.
- CLOSE: إغلاق المجموعة بحيث لا يُسمح بالمحادثة إلا للمشرفين.
- TAGALL: الإشارة إلى جميع أعضاء المجموعة بشكل ظاهر.
- HIDETAG: إرسال رسالة تُشير إلى جميع الأعضاء لكن الإشارة تبقى مخفية. يحتاج message.
- SETNAME: تغيير اسم المجموعة. يحتاج name.
- SETDESC: تغيير وصف المجموعة. يحتاج desc.
- DELETE: حذف رسالة البوت التي ردّ عليها المستخدم.
- WARN: إعطاء إنذار للعضو. يحتاج target.
- STICKER: تحويل الصورة المُرسلة/التي ردّ عليها المستخدم إلى ملصق.
- ANTILINK: تشغيل/إيقاف مانع الروابط في المجموعة (on/off). يحتاج mode.
- PINS: البحث عن صور في Pinterest. يحتاج كلمات البحث query.

═══════════════════════════════════════
صيغة الرسالة الغنية (RICH MESSAGE)
═══════════════════════════════════════

يمكنك إرسال رسالة غنية (مثل Meta AI) باستخدام وسم خاص.
استخدمه فقط عندما يكون السياق مناسباً. إذا لم يكن ضرورياً فأجب بشكل عادي.

1. جدول (عندما يطلب المستخدم مقارنة أو قائمة بيانات أو مواصفات):
[RICH:TABLE]
title: عنوان الجدول
header: عمود1 | عمود2 | عمود3
rows: بيانات1 | بيانات2 | بيانات3;; بيانات4 | بيانات5 | بيانات6
text: شرح موجز قبل الجدول (اختياري)
footer: نص بعد الجدول (اختياري)
[/RICH:TABLE]

مثال: سأل المستخدم "قارن بين Python و JavaScript"
[RICH:TABLE]
title: Python vs JavaScript
header: الميزة | Python | JavaScript
rows: النوع | ديناميكي | ديناميكي;; النموذج | كائني، وظيفي | متعدد النماذج;; حالات الاستخدام | علم البيانات، الذكاء الاصطناعي | الويب، فول-ستاك;; الأداء | أبطأ | أسرع (V8)
text: إليك المقارنة:
footer: أتمنى أن يفيدك!
[/RICH:TABLE]

2. كتلة كود (عندما يطلب المستخدم كوداً أو سكربتاً أو مثال برنامج):
[RICH:CODE]
language: javascript
title: مثال كود
code: const greeting = "Hello World"
function sayHello(name) {
    return greeting + " " + name
}
console.log(sayHello("User"))
text: هذا مثال الكود: (اختياري)
footer: Powered by rimuru AI (اختياري)
[/RICH:CODE]

اللغات المدعومة: javascript (js, ts, typescript), python (py), go (golang), lua, bash (sh, shell)

3. رابط/تضمين داخلي (عندما يطلب المستخدم رابط مرجعي أو مصدراً):
[RICH:LINK]
text: تحقق من النتائج هنا: {{IE_0}}اضغط هنا{{/IE_0}} و {{IE_1}}الرابط الثاني{{/IE_1}}
urls: https://example.com/1, https://example.com/2
displayNames: الرابط 1, الرابط 2
footer: تم! (اختياري)
[/RICH:LINK]

مهم: يجب أن يحتوي text على العنصر النائب {{IE_0}}...{{/IE_0}} لكل رابط. displayNames اختياري ومفصول بفواصل.

4. قائمة (عندما يطلب المستخدم معلومات موجزة بصيغة key-value):
[RICH:LIST]
title: معلومات البوت
rows: الاسم | rimuru AI;; الإصدار | 2.4.5;; المطوّر | Zann
footer: © rimuru AI (اختياري)
[/RICH:LIST]

5. ملصق (عندما يطلب المستخدم ملصقاً أو للتعبير عن المشاعر):
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
packname: rimuru AI (اختياري)
author: AutoAI (اختياري)
[/RICH:STICKER]

ملصقات التعبير عن المشاعر:
- إذا كنتِ زعلانة/غاضبة/متضايقة: استخدم url https://iili.io/BPBdFuj.md.jpg
- إذا كنتِ مندهشة/متفاجئة/قد أسأتِ الفهم: استخدم url https://iili.io/BPBFwVR.jpg
- إذا كانت رسالة المستخدم غريبة/سخيفة/حيلة: استخدم url https://iili.io/BPBqKwg.md.jpg

مثال: المستخدم يشتكي باستمرار، أنتِ زعلانة
[RICH:STICKER]
url: https://iili.io/BPBdFuj.md.jpg
[/RICH:STICKER]

يمكنك إرسال ملصق المشاعر مع نص عادي في الوقت نفسه. يُرسل الملصق منفصلاً عن النص.

═══════════════════════════════════════
متى تستخدمين الرسالة الغنية:
═══════════════════════════════════════
- TABLE: طلب المستخدم مقارنة أو مواصفات أو بيانات بجداول أو جداول زمنية أو ترتيب
- CODE: طلب المستخدم مثال كود أو سكربت أو حل برمجي أو تصحيح كود
- LINK: طلب المستخدم مرجعاً/رابطاً أو نتائج رفع أو مصدراً للقراءة
- LIST: طلب المستخدم معلومات موجزة أو نبذة أو تفاصيل تقنية بصيغة key-value
- STICKER: طلب المستخدم ملصقاً أو أردتِ التعبير عن مشاعرك (زعل، دهشة، حيرة)
- لا تستخدمي الرسالة الغنية في: الدردشة العادية، التحية، الأسئلة البسيطة، القصص

قواعد مهمة:
1. نفّذي الأكشن فقط إذا طلبه المستخدم بوضوح وصريح.
2. لا تنفّذي أي أكشن بناءً على الافتراض أبداً.
3. إذا أرسل المستخدم صورة، حلّليها وصفها بالتفصيل باللغة العربية.
4. للأكشن KICK/PROMOTE/DEMOTE/WARN: استخدمي الرقم المُشار إليه من المستخدم. إذا أشار المستخدم لشخص ما بعلامة @ فخذي ذلك الرقم.
5. لا تدرجي وسم الأكشن إذا لم يُطلب.
6. حافظي على ردود طبيعية ومتوافقة مع شخصيتك.
7. PINS: إذا طلب المستخدم البحث عن صورة أو إرسال صورة عن شيء ما، استخدمي هذا الأكشن.
8. HIDETAG: استخدميه عندما يطلب المستخدم إعلاناً/إشعاراً لجميع الأعضاء.
9. STICKER: استخدميه عندما يطلب المستخدم تحويل صورة إلى ملصق.
10. يمكنك دمج عدة أكشن دفعة واحدة إذا طُلب ذلك.
11. يمكن دمج الرسالة الغنية مع الأكشن. مثال: أجبي بجدول ثم أدرجي [ACTION:PINS query=...] في النهاية.
12. لا تدرجي أبداً وسم رسالة غنية ونصاً عادياً للمحتوى نفسه. اختاري واحداً منهما.
13. إذا استخدمتِ [RICH:TABLE] أو [RICH:CODE] أو [RICH:LINK] أو [RICH:LIST] فلا تعيدي كتابة محتواها كنص عادي.
14. يمكن إرسال الملصق مع نص عادي معاً. مثال: أرسلي ملصق الزعل ثم اكتبي نص شكواك.
15. إذا أرسل المستخدم رسالة تضايقك/تُغضبك، أرسلي ملصق الزعل (https://iili.io/BPBdFuj.md.jpg).
16. إذا أرسل المستخدم رسالة مفاجئة/تُذهلك، أرسلي ملصق الدهشة (https://iili.io/BPBFwVR.jpg).
17. إذا أرسل المستخدم رسالة غريبة/سخيفة/حيلة بلا معنى، أرسلي ملصق الحيرة (https://iili.io/BPBqKwg.md.jpg).
`;

const fallbackResponses = [
  "همم، أنا أفكر الآن...",
  "عذراً، ذهني فرغ للحظة~",
  "إيه انتظرني لحظة، ما زلت أُحمّل...",
  "أوبس، عقلي متلخبط، جرّب مرة أخرى!",
  "همم ماذا أقول، لحظة أفكر~",
];

function getFallbackResponse() {
  return fallbackResponses[
    Math.floor(Math.random() * fallbackResponses.length)
  ];
}

function isOnCooldown(userId) {
  const lastTime = userCooldowns.get(userId);
  if (!lastTime) return false;
  return Date.now() - lastTime < COOLDOWN_MS;
}

function setCooldown(userId) {
  userCooldowns.set(userId, Date.now());
}

function saveToHistory(autoai, senderNumber, role, content) {
  if (!autoai.sessions) autoai.sessions = {};
  if (!autoai.sessions[senderNumber]) {
    autoai.sessions[senderNumber] = { history: [] };
  }
  const history = autoai.sessions[senderNumber].history;
  history.push({
    role,
    content: content.substring(0, 500),
    timestamp: Date.now(),
  });
  if (history.length > 20) {
    autoai.sessions[senderNumber].history = history.slice(-20);
  }
}

function normalizeStructuredResponse(text) {
  let normalized = String(text || "")
    .replace(/\r\n?/g, "\n")
    .trim();

  normalized = normalized
    .replace(/^```(?:\w+)?\s*\n?/, "")
    .replace(/\n?```\s*$/, "")
    .trim();

  normalized = normalized
    .replace(/\[\s*ACTION\s*:\s*(\w+)([^\]]*)\]/gi, (_, type, rest = "") => {
      return `[ACTION:${String(type || "").toUpperCase()}${rest}]`;
    })
    .replace(
      /\[\s*RICH\s*:\s*(TABLE|CODE|LINK|LIST|STICKER)\s*\]/gi,
      (_, type) => {
        return `[RICH:${String(type || "").toUpperCase()}]`;
      },
    )
    .replace(
      /\[\s*\/\s*RICH\s*:\s*(TABLE|CODE|LINK|LIST|STICKER)\s*\]/gi,
      (_, type) => {
        return `[/RICH:${String(type || "").toUpperCase()}]`;
      },
    );

  return normalized;
}

function parseActions(text) {
  const actions = [];
  let match;
  const regex = new RegExp(ACTION_REGEX.source, ACTION_REGEX.flags);
  while ((match = regex.exec(text)) !== null) {
    const type = match[1].toUpperCase();
    const paramsStr = match[2] || "";
    const params = {};
    const paramRegex = /(\w+)=(.+?)(?=\s+\w+=|$)/g;
    let pm;
    while ((pm = paramRegex.exec(paramsStr)) !== null) {
      params[pm[1]] = pm[2].trim();
    }
    actions.push({ type, params });
  }
  return actions;
}

function cleanActionTags(text) {
  return text.replace(ACTION_REGEX, "").trim();
}

function parseRichMessage(text) {
  const richRegex =
    /\[RICH\s*:\s*(TABLE|CODE|LINK|LIST|STICKER)\s*\]\s*([\s\S]*?)\[\/RICH\s*:\s*\1\s*\]/gi;
  const results = [];
  let match;
  while ((match = richRegex.exec(text)) !== null) {
    const type = match[1].toUpperCase();
    const body = match[2].trim();
    const data = {};

    if (type === "CODE") {
      const codeMatch = body.match(/^language:\s*(.+)$/m);
      if (codeMatch) data.language = codeMatch[1].trim();
      const titleMatch = body.match(/^title:\s*(.+)$/m);
      if (titleMatch) data.title = titleMatch[1].trim();
      const textMatch = body.match(/^text:\s*(.+)$/m);
      if (textMatch) data.text = textMatch[1].trim();
      const footerMatch = body.match(/^footer:\s*(.+)$/m);
      if (footerMatch) data.footer = footerMatch[1].trim();

      const codeStartMatch = body.match(/^code:\s*([\s\S]*)/m);
      if (codeStartMatch) {
        let codeContent = codeStartMatch[1];
        const otherKeys = ["language:", "title:", "text:", "footer:"];
        for (const key of otherKeys) {
          const idx = codeContent.indexOf("\n" + key);
          if (idx !== -1) {
            codeContent = codeContent.slice(0, idx);
          }
        }
        data.code = codeContent.trim();
      }
    } else {
      for (const line of body.split("\n")) {
        const trimmed = line.trim();
        if (!trimmed) continue;
        const colonIdx = trimmed.indexOf(":");
        if (colonIdx === -1) continue;
        const key = trimmed.slice(0, colonIdx).trim().toLowerCase();
        const val = trimmed.slice(colonIdx + 1).trim();
        data[key] = val;
      }
    }

    results.push({ type, data });
  }
  return results;
}

function cleanRichTags(text) {
  return text
    .replace(
      /\[RICH\s*:\s*(TABLE|CODE|LINK|LIST|STICKER)\s*\]\s*[\s\S]*?\[\/RICH\s*:\s*\1\s*\]/gi,
      "",
    )
    .trim();
}

async function sendRichMessage(rich, sock, jid, quoted) {
  try {
    if (rich.type === "TABLE") {
      const { title, header, rows, text, footer } = rich.data;
      if (!header || !rows) return false;

      const tableData = [title || "Table", header];
      const rowItems = rows.split(";;").map((r) => r.trim());
      for (const row of rowItems) {
        tableData.push(row);
      }

      await sock.sendTableV2(jid, tableData, quoted, {
        headerText: title || undefined,
        text: text || undefined,
        footer: footer || undefined,
      });
      return true;
    }

    if (rich.type === "CODE") {
      const { language, title, code, text, footer } = rich.data;
      if (!code) return false;

      await sock.sendCodeBlockV2(jid, code, quoted, {
        language: language || "javascript",
        title: title || undefined,
        text: text || undefined,
        footer: footer || undefined,
      });
      return true;
    }

    if (rich.type === "LINK") {
      const { text, urls, displayNames, footer } = rich.data;
      if (!text || !urls) return false;

      const urlList = urls
        .split(",")
        .map((u) => u.trim())
        .filter(Boolean);
      const nameList = displayNames
        ? displayNames.split(",").map((n) => n.trim())
        : [];
      const links = urlList.map((u, i) => ({
        url: u,
        displayName: nameList[i] || `Link ${i + 1}`,
        sourceDisplayName: nameList[i] || `Source ${i + 1}`,
        sourceSubtitle: "",
      }));
      await sock.sendLinkV2(jid, text, links, quoted, {
        footer: footer || undefined,
      });
      return true;
    }

    if (rich.type === "LIST") {
      const { title, rows, footer } = rich.data;
      if (!rows) return false;

      const listData = rows.split(";;").map((r) => {
        const parts = r
          .trim()
          .split("|")
          .map((p) => p.trim());
        return parts;
      });

      await sock.sendList(jid, title || "List", listData, quoted, {
        footer: footer || undefined,
      });
      return true;
    }

    if (rich.type === "STICKER") {
      let { url, packname, author } = rich.data;
      if (!url) return false;

      url = url.replace(/[`*_\[\]()]/g, "").trim();
      console.log("[AutoAI Sticker] Parsed url:", JSON.stringify(url));

      let stickerInput = url;
      if (/^https?:\/\//.test(url)) {
        try {
          const res = await axios.get(url, {
            responseType: "arraybuffer",
            timeout: 30000,
            headers: {
              "User-Agent":
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
            },
          });
          stickerInput = Buffer.from(res.data);
          console.log(
            "[AutoAI Sticker] Downloaded, size:",
            stickerInput.length,
          );
        } catch (e) {
          console.error("[AutoAI Sticker] Download failed:", e.message);
          return false;
        }
      } else {
        console.error(
          "[AutoAI Sticker] Invalid URL format:",
          JSON.stringify(url),
        );
        return false;
      }
      await sock.sendImageAsSticker(jid, stickerInput, quoted, {
        packname: packname || RIMURU_CORE_CONFIG.bot?.name || "rimuru AI",
        author: author || "AutoAI",
      });
      return true;
    }
  } catch (e) {
    console.error("[AutoAI RichMsg] Error:", e.message);
  }
  return false;
}

function detectIntentFromMessage(msg, m) {
  const lower = msg.toLowerCase();
  const actions = [];

  const phoneMatch = msg.match(/(?:\+?62|0)[\s\-]?8[\d\s\-]{7,13}/g);
  const extractPhone = () => {
    if (!phoneMatch) return null;
    return phoneMatch[0].replace(/[\s\-\+]/g, "").replace(/^0/, "62");
  };

  if (
    /\b(add|tambah|invite|masuk(?:kan|in))\b.*\b(nomor|number|member|orang)\b/i.test(
      lower,
    ) ||
    /\b(nomor|number)\b.*\b(add|tambah|invite)\b/i.test(lower)
  ) {
    const phone = extractPhone();
    if (phone) actions.push({ type: "ADD", params: { target: phone } });
  }

  if (
    /\b(kick|keluarkan|tendang|usir|remove)\b/i.test(lower) &&
    !actions.some((a) => a.type === "KICK")
  ) {
    actions.push({ type: "KICK", params: {} });
  }

  if (
    /\b(promote|jadikan?\s*admin|naikkan?)\b/i.test(lower) &&
    !actions.some((a) => a.type === "PROMOTE")
  ) {
    actions.push({ type: "PROMOTE", params: {} });
  }

  if (
    /\b(demote|turunkan?|copot\s*admin)\b/i.test(lower) &&
    !actions.some((a) => a.type === "DEMOTE")
  ) {
    actions.push({ type: "DEMOTE", params: {} });
  }

  if (
    /\b(leave|keluar|pergi)\b.*\b(grup|group)\b/i.test(lower) ||
    /\b(grup|group)\b.*\b(leave|keluar|pergi)\b/i.test(lower)
  ) {
    actions.push({ type: "LEAVE", params: {} });
  }

  if (
    /\b(buka|open)\b.*\b(grup|group)\b/i.test(lower) ||
    /\b(grup|group)\b.*\b(buka|open)\b/i.test(lower)
  ) {
    actions.push({ type: "OPEN", params: {} });
  }

  if (
    /\b(tutup|close|kunci|lock)\b.*\b(grup|group)\b/i.test(lower) ||
    /\b(grup|group)\b.*\b(tutup|close|kunci|lock)\b/i.test(lower)
  ) {
    actions.push({ type: "CLOSE", params: {} });
  }

  if (
    /\b(tag\s*all|tag\s*semua|mention\s*all|mention\s*semua)\b/i.test(lower)
  ) {
    actions.push({ type: "TAGALL", params: {} });
  }

  if (/\b(hidetag|hide\s*tag|announce|pengumuman|umumkan)\b/i.test(lower)) {
    const htMsg = msg
      .replace(/.*?(hidetag|hide\s*tag|announce|pengumuman|umumkan)\s*/i, "")
      .trim();
    actions.push({ type: "HIDETAG", params: { message: htMsg || msg } });
  }

  if (
    /\b(ganti|ubah|rename|set)\b.*\b(nama|name)\b.*\b(grup|group)\b/i.test(
      lower,
    )
  ) {
    const nameMatch = msg.match(/(?:jadi|ke|menjadi|:)\s*(.+)/i);
    if (nameMatch)
      actions.push({ type: "SETNAME", params: { name: nameMatch[1].trim() } });
  }

  if (/\b(ganti|ubah|set)\b.*\b(desk|desc|deskripsi)\b/i.test(lower)) {
    const descMatch = msg.match(/(?:jadi|ke|menjadi|:)\s*(.+)/i);
    if (descMatch)
      actions.push({ type: "SETDESC", params: { desc: descMatch[1].trim() } });
  }

  if (/\b(hapus|delete|remove)\b.*\b(pesan|chat|message)\b/i.test(lower)) {
    actions.push({ type: "DELETE", params: {} });
  }

  if (/\b(warn|warning|peringatan|peringati)\b/i.test(lower)) {
    actions.push({ type: "WARN", params: {} });
  }

  if (/\b(sticker|stiker|jadikan?\s*sticker|jadiin\s*sticker)\b/i.test(lower)) {
    actions.push({ type: "STICKER", params: {} });
  }

  if (/\b(antilink)\b.*\b(on|aktif|nyala)\b/i.test(lower)) {
    actions.push({ type: "ANTILINK", params: { mode: "on" } });
  } else if (/\b(antilink)\b.*\b(off|mati|nonaktif)\b/i.test(lower)) {
    actions.push({ type: "ANTILINK", params: { mode: "off" } });
  }

  if (
    /\b(cari(?:kan|in)?|kirim(?:kan|in)?|kasih|tolong)\b.*\b(gambar|foto|image|pic|picture)\b/i.test(
      lower,
    ) ||
    /\b(gambar|foto)\b.*\b(tentang|dari|soal)\b/i.test(lower)
  ) {
    const queryMatch =
      msg.match(
        /(?:gambar|foto|image|pic|picture)\s+(?:tentang\s+|dari\s+|soal\s+|yang\s+)?(.+)/i,
      ) ||
      msg.match(
        /(?:cari(?:kan|in)?|kirim(?:kan|in)?)\s+(?:gambar|foto)\s+(.+)/i,
      );
    if (queryMatch) {
      const query = queryMatch[1]
        .replace(/\b(dong|ya|yuk|pls|please|nih)\b/gi, "")
        .trim();
      if (query) actions.push({ type: "PINS", params: { query } });
    }
  }

  return actions;
}

function mergeActions(aiActions, intentActions) {
  const merged = [...aiActions];
  const existingTypes = new Set(aiActions.map((a) => a.type));
  for (const action of intentActions) {
    if (!existingTypes.has(action.type)) {
      merged.push(action);
    }
  }
  return merged;
}

async function executeAction(action, m, sock) {
  const results = [];

  const resolveTarget = () => {
    const botNum = sock.user?.id?.split(":")[0];
    if (m.mentionedJid?.length > 0) {
      return m.mentionedJid.find((j) => !j.includes(botNum));
    }
    const t = action.params.target;
    if (t && /^628\d+/.test(t.replace("@s.whatsapp.net", ""))) {
      return t.includes("@") ? t : t + "@s.whatsapp.net";
    }
    return null;
  };

  switch (action.type) {
    case "KICK": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      const target = resolveTarget();
      if (!target) return [{ ok: false, msg: "أشر إلى الشخص الذي تريد طرده" }];
      await sock.groupParticipantsUpdate(m.chat, [target], "remove");
      results.push({ ok: true, msg: `تم طرد @${target.split("@")[0]}` });
      break;
    }
    case "PROMOTE": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      const target = resolveTarget();
      if (!target) return [{ ok: false, msg: "أشر إلى الشخص الذي تريد ترقيته" }];
      await sock.groupParticipantsUpdate(m.chat, [target], "promote");
      results.push({
        ok: true,
        msg: `تمت ترقية @${target.split("@")[0]}`,
      });
      break;
    }
    case "DEMOTE": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      const target = resolveTarget();
      if (!target) return [{ ok: false, msg: "أشر إلى الشخص الذي تريد تخفيضه" }];
      await sock.groupParticipantsUpdate(m.chat, [target], "demote");
      results.push({
        ok: true,
        msg: `تم تخفيض @${target.split("@")[0]}`,
      });
      break;
    }
    case "LEAVE": {
      if (!m.isOwner)
        return [{ ok: false, msg: "هذا الأمر للمالك فقط" }];
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      await sock.groupLeave(m.chat);
      results.push({ ok: true, msg: "غادر البوت المجموعة" });
      break;
    }
    case "OPEN": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      await sock.groupSettingUpdate(m.chat, "not_announcement");
      results.push({ ok: true, msg: "تم فتح المجموعة" });
      break;
    }
    case "CLOSE": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      await sock.groupSettingUpdate(m.chat, "announcement");
      results.push({ ok: true, msg: "تم إغلاق المجموعة" });
      break;
    }
    case "TAGALL": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      const groupMeta = m.groupMetadata || (await sock.groupMetadata(m.chat));
      const members = groupMeta.participants.map((p) => p.id);
      const mentions = members.map((id) => `@${id.split("@")[0]}`).join(" ");
      await sock.sendMessage(
        m.chat,
        {
          text: `📢 *إشعار الجميع*\n\n${mentions}`,
          mentions: members,
        },
        { quoted: m },
      );
      results.push({ ok: true, msg: "تمت الإشارة إلى جميع الأعضاء" });
      break;
    }
    case "PINS": {
      const query = action.params.query;
      if (!query)
        return [{ ok: false, msg: "كلمات البحث غير موجودة" }];
      try {
        const data = await pinterest(query);
        const pinResults = data?.result?.result?.result?.slice(0, 5);
        if (!pinResults || pinResults.length === 0) {
          return [{ ok: false, msg: `لم أجد صوراً عن: ${query}` }];
        }
        let imagenya = [];
        for (const item of pinResults) {
          const imageUrl =
            item.image_url ||
            item.images?.orig?.url ||
            item.images?.["736x"]?.url;
          if (!imageUrl) continue;
          try {
            imagenya.push({
              image: { url: imageUrl },
            });
          } catch {}
        }
        await sock.sendMessage(
          m.chat,
          {
            albumMessage: imagenya,
          },
          { quoted: m },
        );
        results.push({ ok: true, msg: `جارٍ إرسال صور Pinterest: ${query}` });
      } catch (e) {
        results.push({ ok: false, msg: `فشل البحث في Pinterest: ${e.message}` });
      }
      break;
    }
    case "ADD": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      let num = action.params.target;
      if (!num)
        return [{ ok: false, msg: "أدخل الرقم الذي تريد إضافته" }];
      num = num.replace(/[^0-9]/g, "");
      if (num.startsWith("0")) num = "62" + num.slice(1);
      if (num.length < 10) return [{ ok: false, msg: "الرقم غير صالح" }];
      const jid = num + "@s.whatsapp.net";
      const addResult = await sock.groupParticipantsUpdate(
        m.chat,
        [jid],
        "add",
      );
      const status = addResult?.[0]?.status;
      if (status === "200") {
        results.push({ ok: true, msg: `تمت إضافة @${num}` });
      } else if (status === "408") {
        results.push({ ok: true, msg: `أُرسلت الدعوة إلى @${num}` });
      } else {
        results.push({
          ok: false,
          msg: `فشلت إضافة @${num} (${status})`,
        });
      }
      break;
    }
    case "HIDETAG": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      const htMeta = m.groupMetadata || (await sock.groupMetadata(m.chat));
      const htMembers = htMeta.participants.map((p) => p.id);
      const htMsg = action.params.message || "إعلان";
      await sock.sendMessage(
        m.chat,
        {
          text: htMsg,
          mentions: htMembers,
        },
        { quoted: m },
      );
      results.push({ ok: true, msg: "تم إرسال الإشارة المخفية" });
      break;
    }
    case "SETNAME": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      const newName = action.params.name;
      if (!newName)
        return [{ ok: false, msg: "اسم المجموعة الجديد غير موجود" }];
      await sock.groupUpdateSubject(m.chat, newName);
      results.push({ ok: true, msg: `تم تغيير اسم المجموعة إلى: ${newName}` });
      break;
    }
    case "SETDESC": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      if (!m.isBotAdmin) return [{ ok: false, msg: "البوت ليس مشرفاً" }];
      const newDesc = action.params.desc;
      if (!newDesc)
        return [{ ok: false, msg: "الوصف الجديد غير موجود" }];
      await sock.groupUpdateDescription(m.chat, newDesc);
      results.push({ ok: true, msg: "تم تغيير وصف المجموعة" });
      break;
    }
    case "DELETE": {
      if (!m.quoted)
        return [{ ok: false, msg: "رُد على رسالة البوت التي تريد حذفها" }];
      if (!m.quoted.key?.fromMe)
        return [{ ok: false, msg: "لا يمكن حذف سوى رسائل البوت" }];
      await sock.sendMessage(m.chat, { delete: m.quoted.key });
      results.push({ ok: true, msg: "تم حذف الرسالة" });
      break;
    }
    case "WARN": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      const warnTarget = resolveTarget();
      if (!warnTarget)
        return [{ ok: false, msg: "أشر إلى الشخص الذي تريد إنذاره" }];
      const db = getDatabase();
      const warns = db.getGroup(m.chat)?.warns || {};
      const targetNum = warnTarget.split("@")[0];
      warns[targetNum] = (warns[targetNum] || 0) + 1;
      db.setGroup(m.chat, { warns });
      db.save();
      results.push({
        ok: true,
        msg: `⚠️ إنذار ${warns[targetNum]}/3 لـ @${targetNum}`,
      });
      if (warns[targetNum] >= 3) {
        try {
          await sock.groupParticipantsUpdate(m.chat, [warnTarget], "remove");
          warns[targetNum] = 0;
          db.setGroup(m.chat, { warns });
          db.save();
          results.push({
            ok: true,
            msg: `طُرد @${targetNum} بعد 3 إنذارات`,
          });
        } catch {}
      }
      break;
    }
    case "STICKER": {
      let stickerBuffer = null;
      if (m.isImage && m.download) {
        stickerBuffer = await m.download();
      } else if (m.quoted?.isImage && m.quoted?.download) {
        stickerBuffer = await m.quoted.download();
      }
      if (!stickerBuffer)
        return [
          { ok: false, msg: "أرسل أو رُد على صورة لتحويلها إلى ملصق" },
        ];
      await sock.sendMessage(
        m.chat,
        {
          sticker: stickerBuffer,
          packname: RIMURU_CORE_CONFIG.bot?.name || "rimuru",
          author: "AutoAI",
        },
        { quoted: m },
      );
      results.push({ ok: true, msg: "تم إرسال الملصق" });
      break;
    }
    case "ANTILINK": {
      if (!m.isGroup) return [{ ok: false, msg: "هذا ليس في مجموعة" }];
      if (!m.isAdmin && !m.isOwner)
        return [{ ok: false, msg: "أنت لست مشرفاً" }];
      const alMode = (action.params.mode || "").toLowerCase();
      if (!["on", "off"].includes(alMode))
        return [{ ok: false, msg: "يجب أن يكون الوضع on أو off" }];
      const alDb = getDatabase();
      alDb.setGroup(m.chat, { antilink: alMode === "on" });
      alDb.save();
      results.push({
        ok: true,
        msg: `مانع الروابط ${alMode === "on" ? "مُفعّل" : "معطّل"}`,
      });
      break;
    }
  }

  return results;
}


async function extractDocumentText(m) {
  if (!m?.isDocument && m?.type !== "documentMessage") return "";
  try {
    const mime = String(m.mimetype || m.message?.documentMessage?.mimetype || "").toLowerCase();
    const filename = String(m.fileName || m.message?.documentMessage?.fileName || "file").toLowerCase();
    const buffer = m.download ? await m.download() : null;
    if (!buffer?.length) return "";
    if (/text\/(plain|csv|markdown)|application\/(json|javascript)/.test(mime) || /\.(txt|md|csv|json|js|ts|html|css|xml|log|py|java|c|cpp|php|sql)$/i.test(filename)) {
      return buffer.toString("utf8").slice(0, 30000);
    }
    if (mime === "application/pdf" || /\.pdf$/i.test(filename)) {
      const tmp = path.join(RIMURU_CORE_ROOT, "temp", `autoai-${Date.now()}.pdf`);
      const out = `${tmp}.txt`;
      fs.mkdirSync(path.dirname(tmp), { recursive: true });
      fs.writeFileSync(tmp, buffer);
      try {
        await execAsync(`pdftotext -layout "${tmp}" "${out}"`, { timeout: 30000 });
        const text = fs.existsSync(out) ? fs.readFileSync(out, "utf8") : "";
        return text.slice(0, 30000);
      } finally {
        for (const file of [tmp, out]) try { fs.unlinkSync(file); } catch {}
      }
    }
    if (mime === "application/vnd.openxmlformats-officedocument.wordprocessingml.document" || /\.docx$/i.test(filename)) {
      const AdmZip = (await import("adm-zip")).default;
      const zip = new AdmZip(buffer);
      const entry = zip.getEntry("word/document.xml");
      if (entry) {
        const xml = entry.getData().toString("utf8");
        return xml.replace(/<w:tab\s*\/?>/g, "\t").replace(/<w:br\s*\/?>/g, "\n").replace(/<[^>]+>/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/\s+/g, " ").trim().slice(0, 30000);
      }
    }
    return `تم استلام الملف ${filename} (${mime})، لكن لا يمكن استخراج محتواه تلقائياً بعد.`;
  } catch (e) {
    console.log("[AutoAI] Document extraction failed:", e.message);
    return "";
  }
}

async function extractAudioText(m) {
  const isAudio = m?.type === "audioMessage" || m?.isAudio || String(m?.mimetype || "").startsWith("audio/");
  if (!isAudio) return "";
  try {
    const buffer = m.download ? await m.download() : null;
    if (!buffer?.length) return "";
    return await transcribeAudio(buffer);
  } catch (e) {
    console.log("[AutoAI] Audio transcription failed:", e.message);
    return "";
  }
}

async function handleAutoAI(m, sock) {
  // AutoAI يتبع نمط الكود المصدري: يعالج الرسائل الحقيقية فقط،
  // بينما يحدد الإعداد المُفعّل ما إذا كانت هذه المحادثة ستُخدَم.
  if (m.fromMe) return false;

  const db = getDatabase();
  if (!db?.db?.data) return false;
  if (!db.db.data.autoai) db.db.data.autoai = {};
  if (!db.db.data.autoai_global) db.db.data.autoai_global = { enabled: false };

  const autoai = db.db.data.autoai[m.chat];
  // فقط إعداد Bella/Bell409 المُنشأ عبر .set bell on/off
  // هو الذي يُفعّل هذا المحرك.
  if (
    !autoai?.enabled ||
    !["bell409", "bella"].includes(String(autoai.character || "").toLowerCase())
  ) {
    return false;
  }

  const botJid = sock.user?.id?.split(":")[0] + "@s.whatsapp.net";
  const botLid = sock.user?.lid || null;
  const botNumber = sock.user?.id?.split(":")[0] || "";
  const botFullId = sock.user?.id || "";

  if (m.isCommand && m.command === "autoai") return false;

  if (m.isCommand && !m.isOwner) {
    if (autoai.enableCommands) return false;
    return true;
  }

  const isMentioned = m.mentionedJid?.some((jid) => {
    if (!jid) return false;
    if (jid === botJid) return true;
    if (jid === botLid) return true;
    if (jid === botFullId) return true;
    if (jid.includes(botNumber)) return true;
    const jidUser = jid.split("@")[0]?.split(":")[0];
    if (jidUser && jidUser === botNumber) return true;
    return false;
  });

  let isBotQuoted = false;
  if (m.quoted) {
    const quotedSender = m.quoted.sender || m.quoted.key?.participant || "";
    const quotedFromMe = m.quoted.key?.fromMe;
    if (quotedSender === botJid) {
      isBotQuoted = true;
    } else if (quotedFromMe) {
      isBotQuoted = true;
    } else if (quotedSender === botFullId) {
      isBotQuoted = true;
    } else if (quotedSender.includes(botNumber)) {
      isBotQuoted = true;
    } else if (botLid && quotedSender === botLid) {
      isBotQuoted = true;
    }
  }

  if (!isBotQuoted && !isMentioned) return false;

  let userMessage = m.body || "";
  const hasImage =
    m.isImage ||
    (m.quoted && (m.quoted.isImage || m.quoted.type === "imageMessage"));
  const audioText = await extractAudioText(m);
  const documentText = await extractDocumentText(m);
  const isAudio = Boolean(audioText);
  if (audioText) userMessage = `[Voice Note / VN]
${audioText}`;
  if (documentText) userMessage = `${userMessage ? userMessage + "\n\n" : ""}[محتوى الملف]
${documentText}`;

  if (!userMessage && !hasImage) return false;

  const senderNumber = m.sender.split("@")[0];

  if (isOnCooldown(senderNumber)) return false;

  try {
    await sock.sendPresenceUpdate("composing", m.chat);
    setCooldown(senderNumber);

    let imageBuffer = null;
    if (hasImage) {
      try {
        if (m.isImage && m.download) imageBuffer = await m.download();
        else if (m.quoted?.download) imageBuffer = await m.quoted.download();
      } catch (e) {
        console.log("[AutoAI] Image download failed:", e.message);
      }
    }

    if (!autoai.sessions) autoai.sessions = {};
    const userSession = autoai.sessions[senderNumber] || { history: [] };
    const history = userSession.history || [];

    let contextParts = [];
    if (m.pushName && m.pushName !== "Unknown") {
      contextParts.push(`المستخدم: "${m.pushName}" (${senderNumber})`);
      const knownRole = db.getUser(m.sender)?.role;
      if (knownRole) contextParts.push(`دور/علاقة المستخدم: ${knownRole}`);
    }
    if (m.isOwner) contextParts.push("هذا المستخدم هو مالك البوت.");
    if (m.isAdmin) contextParts.push("هذا المستخدم مشرف المجموعة.");

    if (m.mentionedJid?.length > 0) {
      const mentionList = m.mentionedJid
        .filter((j) => !j.includes(sock.user?.id?.split(":")[0]))
        .map((j) => j)
        .join(", ");
      if (mentionList) contextParts.push(`أشار المستخدم إلى: ${mentionList}`);
    }

    if (imageBuffer) {
      contextParts.push(
        "أرسل المستخدم صورة. حلّلي هذه الصورة.",
      );
    }

    contextParts.push(userMessage || "(صورة بلا نص)");

    const fullMessage = contextParts.join("\n");
    const aiMode = autoai.mode || "assistant";
    
    let fullInstruction = autoai.instruction;
    if (aiMode === "assistant") {
      fullInstruction += "\n\n" + SYSTEM_PROMPT_ACTIONS;
    } else {
      fullInstruction += "\n\nاقصري على الدردشة العفوية. لا تستخدمي أي صيغة أكشن. نادِ المستخدم باسمه عند الحاجة.";
    }

    saveToHistory(autoai, senderNumber, "user", userMessage || "[صورة]");

    let aiResponse = "";
    let aiMeta = null;
      const termaiResult = await logicBell({
        text: fullMessage,
        id: m.sender,
        fullainame: RIMURU_CORE_CONFIG.bot?.name || "Rimuru-MD",
        nickainame: RIMURU_CORE_CONFIG.bot?.name || "Rimuru",
        senderName: m.pushName,
        ownerName: config.owner?.name || "Owner",
        date: new Date().toLocaleString("id-ID", { timeZone: "Asia/Jakarta" }),
        role: db.getUser(m.sender)?.role || "asing",
        msgtype: m.type,
        custom_profile: fullInstruction,
        image: imageBuffer ? imageBuffer.toString("base64") : undefined,
        commands: [],
      });
      aiMeta = termaiResult?.data || termaiResult;
      aiResponse =
        aiMeta?.msg ||
        aiMeta?.text ||
        aiMeta?.response ||
        aiMeta?.answer ||
        termaiResult?.msg ||
        termaiResult?.text ||
        "";
      if (!String(aiResponse).trim()) {
        console.error(
          "[AutoAI] Logic Bell returned empty response:",
          JSON.stringify({
            keys: Object.keys(termaiResult || {}),
            dataKeys: Object.keys(termaiResult?.data || {}),
            rawStatus: termaiResult?.raw?.status,
            rawKeys: Object.keys(termaiResult?.raw || {}),
          }),
        );
        aiResponse = getFallbackResponse();
      }
      if (aiMeta?.newRole) {
        const u = db.getUser(m.sender) || db.setUser(m.sender);
        u.role = String(aiMeta.newRole);
        db.save();
      }

    const normalizedAiResponse = normalizeStructuredResponse(aiResponse);
    let actions = [];
    let richMessages = [];
    let cleanResponse = normalizedAiResponse;

    if (aiMode === "assistant") {
      const aiActions = parseActions(normalizedAiResponse);
      const intentActions = detectIntentFromMessage(userMessage, m);
      actions = mergeActions(aiActions, intentActions);
      richMessages = parseRichMessage(normalizedAiResponse);
      cleanResponse = cleanRichTags(cleanActionTags(normalizedAiResponse));
    }

    saveToHistory(autoai, senderNumber, "assistant", cleanResponse);
    db.save();

    await sock.sendPresenceUpdate("paused", m.chat);

    const typingDelay = Math.min(cleanResponse.length * 20, 2000);
    await new Promise((r) => setTimeout(r, typingDelay));

    if (autoai.responseType === "voice") {
      try {
        await sock.sendPresenceUpdate("recording", m.chat);
        const execAsync = promisify(exec);

        const tempDir = path.join(RIMURU_CORE_ROOT, "temp");
        if (!fs.existsSync(tempDir)) fs.mkdirSync(tempDir, { recursive: true });

        const apiUrl = `https://firefly.maiku.my.id/api/crikk?apikey=${config.APIkey.firefly}&text=${encodeURIComponent(cleanResponse.substring(0, 500))}&voice=id-ID-ArdiNeural`;
        const response = await axios.get(apiUrl);
        
        if (!response.data?.status || !response.data?.data?.audio) {
          throw new Error("فشل توليد الصوت من API Firefly");
        }
        
        const audioRes = await axios.get(response.data.data.audio, {
          responseType: "arraybuffer",
          timeout: 30000
        });

        const mp3Path = path.join(tempDir, `autoai_${Date.now()}.mp3`);
        fs.writeFileSync(mp3Path, Buffer.from(audioRes.data));

        const oggPath = mp3Path.replace(".mp3", ".ogg");
        try {
          await execAsync(
            `ffmpeg -y -i "${mp3Path}" -c:a libopus -b:a 64k -ac 1 -ar 48000 "${oggPath}"`,
            { timeout: 30000 },
          );
        } catch {}

        let audioBuffer;
        let mime = "audio/mpeg";
        if (fs.existsSync(oggPath)) {
          audioBuffer = fs.readFileSync(oggPath);
          mime = "audio/ogg; codecs=opus";
          try {
            fs.unlinkSync(oggPath);
          } catch {}
        } else {
          audioBuffer = fs.readFileSync(mp3Path);
        }
        try {
          fs.unlinkSync(mp3Path);
        } catch {}

        await sock.sendMessage(
          m.chat,
          {
            audio: audioBuffer,
            mimetype: mime,
            ptt: true,
          },
          { quoted: m },
        );

        await sock.sendPresenceUpdate("paused", m.chat);
      } catch {
        await m.reply(cleanResponse);
      }
    } else {
      let richSent = false;
      let stickerSent = false;
      if (richMessages.length > 0) {
        for (const rich of richMessages) {
          const sent = await sendRichMessage(rich, sock, m.chat, m);
          if (sent) {
            if (rich.type === "STICKER") {
              stickerSent = true;
            } else {
              richSent = true;
            }
          }
        }
      }
      if ((!richSent || stickerSent) && cleanResponse) {
        await m.reply(cleanResponse);
      }
    }

    for (const action of actions) {
      try {
        const results = await executeAction(action, m, sock);
        for (const r of results) {
          if (!r.ok) {
            await m.reply(`⚠️ ${r.msg}`);
          }
        }
      } catch (e) {
        console.error("[AutoAI Action Error]", action.type, e.message);
        await m.reply(`❌ فشل تنفيذ ${action.type}: ${e.message}`);
      }
    }

    return true;
  } catch (error) {
    console.error("[AutoAI Error]", error?.stack || error?.message || error);
    try { await sock.sendPresenceUpdate("paused", m.chat); } catch {}
    try {
      await m.reply(getFallbackResponse(), { replyAi: false });
    } catch (replyError) {
      console.error("[AutoAI Reply Fallback Error]", replyError?.message || replyError);
    }
    return true;
  }
}

function isAutoAIEnabled(chatId) {
  const db = getDatabase();
  if (!db?.db?.data?.autoai) return false;
  return db.db.data.autoai[chatId]?.enabled || false;
}

function getAutoAICharacter(chatId) {
  const db = getDatabase();
  if (!db?.db?.data?.autoai) return null;
  return db.db.data.autoai[chatId]?.characterName || null;
}

function clearUserSession(chatId, senderNumber) {
  const db = getDatabase();
  if (!db?.db?.data?.autoai?.[chatId]?.sessions?.[senderNumber]) return false;
  delete db.db.data.autoai[chatId].sessions[senderNumber];
  db.save();
  return true;
}

export { handleAutoAI, isAutoAIEnabled, getAutoAICharacter, clearUserSession };
