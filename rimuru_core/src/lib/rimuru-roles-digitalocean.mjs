...
const DO_DIR = path.join(RIMURU_CORE_ROOT, 'database', 'digitalocean')
...
function addRole(jid, server, role) {
    if (!VALID_SERVERS.includes(server)) return { success: false, error: 'الخادم غير صالح' }
    if (!VALID_ROLES.includes(role)) return { success: false, error: 'الرتبة غير صالحة' }
    
    const number = cleanNumber(jid)
    if (!number) return { success: false, error: 'الرقم غير صالح' }
    
    const existingRole = getUserRole(jid, server)
    if (existingRole) {
        return { success: false, error: `لديه بالفعل رتبة ${existingRole} في ${server.toUpperCase()}` }
    }
    
    const list = loadRoleFile(role, server)
    if (list.includes(number)) {
        return { success: false, error: 'مسجّل بالفعل' }
    }
...
function removeRole(jid, server, role) {
    ...
    if (idx === -1) {
        return { success: false, error: 'غير مسجّل' }
    }
