...
function addRodExp(fischUser, rodKey, amount) {
  ...
  while (rod.exp >= rod.expToNextLevel && rod.level < rod.maxLevel) {
    ...
    levelUp = true;
    statsIncreased.push(`الحظ +1%، السرعة +1%`);
  }
  if (levelUp)
    return `🎣 ارتقت القضبة *${rod.name}* إلى المستوى ${rod.level}!\n✨ ${statsIncreased.join(", ")}`;
  return null;
}
...
function applyJackpotReward(db, fischUser, sender, result) {
  ...
  switch (type) {
    ...
    case "tickets":
      ...
      applied = { desc: `تذاكر غاتشا +${val}`, type };
      break;
    case "tokens":
    ...
    case "unlimited_energi": {
      ...
      applied = { desc: `طاقة غير محدودة!`, type };
      break;
    }
    case "unlimited_limit": {
      ...
      applied = { desc: `حد يومي غير محدود!`, type };
      break;
    }
    default:
