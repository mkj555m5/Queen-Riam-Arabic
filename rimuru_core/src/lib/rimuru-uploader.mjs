...
import { updateAssetAndSave } from './rimuru-asset-manager.mjs';
...
export async function updateAssetUrl(assetKey, buffer, filename = 'image.jpg') {
  ...
  if (!localPath || localPath.startsWith('http')) {
    ...
    const configPath = path.join(RIMURU_CORE_ROOT, 'config.js');
  ...
  const fullPath = path.resolve(RIMURU_CORE_ROOT, localPath);
