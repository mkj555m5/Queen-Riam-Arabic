...
const GENRES = {
    action: {
        label:      'أكشن',
        taglines:   [
            'شخص واحد. مهمة واحدة. بلا رحمة.',
            'أخذوا كل شيء. خطأ فادح.',
            'ثمة حروب تختارك أنت.',
            'المواجهة الأخيرة تبدأ.',
            'وُلد ليقاتل، وتدرّب لينتصر.',
            'للعدالة اسم جديد.',
        ],
    ...
    thriller: {
        ...
        taglines:   [
            'الحقيقة ستقتلك.',
            'لا أحد بريء.',
            'لا تثق بأحد. وخف الجميع.',
            'ثمة أسرار ينبغي أن تبقى مدفونة.',
            'كلما اقتربت، ازداد الظلام.',
            'الجميع مشتبه به.',
        ],
        ratings:    ['R','R','PG-13'],
    ...
    romance: {
        label:      'رومانسي',
        taglines:   [
            'ثمة حكايات حب تتحدى كل الاحتمالات.',
            'حين يكتب القدر الفصل الأخير.',
            'الحب يجد طريقه. دائماً.',
            'حب أقوى من أن يُتجاهل.',
            'في عالم مليء بالضجيج، كان هناك أنت.',
            'الأبدية تبدأ من لحظة.',
        ],
        ratings:    ['PG','PG-13','PG'],
    ...
    scifi: {
        label:      'خيال علمي',
        taglines:   [
            'المستقبل ليس كما تتصور.',
            'ما وراء النجوم تكمن الحقيقة.',
            'أمل البشرية الأخير.',
            'لقد اختار الكون.',
            'حين تتصادم العوالم، يولد الأبطال.',
            'أول تواصل. آخر فرصة.',
        ],
    ...
    horror: {
        ...
        taglines:   [
            ...
            'إنه يعرف اسمك.',
            'الشر لا ينام أبداً.',
            'لا يمكنك الهرب من الظلام.',
            'حين يحلّ الليل، يأتي.',
            'صلِّ ألّا تكون التالي.',
        ],
        ratings:    ['R','R','R','PG-13'],
    ...
    comedy: {
        label:      'كوميدي',
        taglines:   [
            'استعد لتضحك. ثم تضحك أكثر.',
            'الحياة أقصر من أن تكون جاداً.',
            'أطرف مشكلة في العالم.',
            'لا شيء يسير حسب الخطة. أبداً.',
            'حين يسوء كل شيء، اضحك.',
            'مغامرة العمر بلا قصد.',
        ],
...
async function createMoviePoster(opts = {}) {
    ...
    const movieRuntime = runtime || (Math.floor(rng() * 60) + 90) + ' دقيقة';
    const movieDirector = director || 'فيلم من إخراج ' + name.toUpperCase();
    ...
    ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    const creditsLine = `${studio}  ©${movieYear}  جميع الحقوق محفوظة  ·  غير مخصص للتوزيع`;
    ctx.fillText(creditsLine, 24, BOTTOM_Y + 10);
    ctx.font = '8px sans-serif'; ctx.fillStyle = 'rgba(255,255,255,0.18)';
    ctx.fillText('بالتعاون مع BULTERBOT PRODUCTIONS', 24, BOTTOM_Y + 24);
    ctx.restore();
