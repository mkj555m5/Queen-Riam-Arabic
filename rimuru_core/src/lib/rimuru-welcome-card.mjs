...
async function createWideDiscordCard(
  ...
  ctx.fillText("● عضو جديد", textX + 15, 144);
  ...
  ctx.font = "24px Arial";
  ctx.fillText(`انضم إلى: ${groupName}`, textX, 260);

  ctx.fillStyle = "#ffffff";
  ctx.font = "bold 20px Arial";
  ctx.fillText(`الأعضاء: #${memberCount}`, textX, 320);
  ctx.beginPath();
...
async function createGoodbyeCard(username, avatarUrl, groupName, memberCount) {
  ...
  ctx.font = "bold 18px Courier New";
  ctx.fillText("● وداعاً", textX + 15, 144);
  ...
  ctx.font = "24px Arial";
  ctx.fillText(`غادر: ${groupName}`, textX, 260);

  ctx.fillStyle = "#ffcccc";
  ctx.font = "bold 20px Arial";
  ctx.fillText(`المتبقون: #${memberCount}`, textX, 320);
...
async function createWelcomeCardV4(
  ...
  ctx.fillStyle = "#00d2ff";
  ctx.fillText("أهلاً وسهلاً", textStart, 160);
  ...
  ctx.font = "30px Arial";
  ctx.fillStyle = "#a0a0a0";
  ctx.fillText("إلى " + groupName, textStart, 280);

  // Member Count Tag
  const tagY = 320;
  const tagText = `العضو #${memberCount}`;
...
async function createGoodbyeCardV4(
  ...
  ctx.fillText("وداعاً", textStart, 160);
  ...
  ctx.font = "30px Arial";
  ctx.fillStyle = "#a0a0a0";
  ctx.fillText("من " + groupName, textStart, 280);
  const tagY = 320;
  const tagText = `المتبقون #${memberCount}`;
  ctx.font = "bold 24px Arial";
  const tagWidth = ctx.measureText(tagText).width + 40;
