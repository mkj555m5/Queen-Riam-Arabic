...
const DATABASE_DIR = path.join(RIMURU_CORE_ROOT, 'database')
const TEMP_DIR = path.join(RIMURU_CORE_ROOT, 'temp')
    ...
    return new Promise((resolve, reject) => {
        ...
        if (fs.existsSync(DATABASE_DIR)) {
            fs.promises.readdir(DATABASE_DIR, { withFileTypes: true }).then(entries => {
                ...
                const storageDir = path.join(RIMURU_CORE_ROOT, 'storage')
                ...
                const rootDbFile = path.join(RIMURU_CORE_ROOT, 'db.json')
                if (fs.existsSync(rootDbFile)) {
                    archive.file(rootDbFile, { name: 'db.json' })
                }
                
                const mainDbFile = path.join(RIMURU_CORE_ROOT, 'database', 'main', 'db.json')
...
async function sendStoreBackup(sock) {
    ...
    try {
        ...
        const caption = 
            `🗃️ *ꜱᴛᴏʀᴇ ʙᴀᴄᴋᴜᴘ*\n\n` +
            `╭┈┈⬡「 📋 *ɪɴꜰᴏ* 」\n` +
            `┃ 📅 الوقت: ${timeHelper.formatDateTime('DD MMMM YYYY HH:mm:ss')} بتوقيت جاكرتا\n` +
            `┃ 📦 الحجم: ${sizeDisplay}\n` +
            `┃ 📁 الملفات: ${backupInfo.fileCount}\n` +
            `┃ 🔖 المخطط: v${SCHEMA_VERSION}\n` +
            `╰┈┈┈┈┈┈┈┈⬡\n\n` +
            `> نسخة احتياطية آمنة الأنواع ومتوافقة مع الإصدارات القادمة.\n` +
            `> ${RIMURU_CORE_CONFIG.bot?.name || 'rimuru-AI'} Store Backup System`
