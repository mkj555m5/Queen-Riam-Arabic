...
// التحقق من إمكانية الالتقاط للاعب
...
async function renderBoard(game, selectedPos = null, validMoves = [], lastMove = null) {
    ...
    ctx.shadowBlur = 10
    ctx.fillText('🦋 RIMURU CHECKERS 🦋', W/2, 45)
    ctx.shadowBlur = 0

    ctx.fillStyle = THEME.textSecondary
    ctx.font = '12px "Segoe UI"'
    ctx.fillText('"ضامة - التهم كل قطع الخصم!"', W/2, 70)
    ...
    ctx.fillText(`🎮 الدور: @${turnName}`, W/2, H - 25)

    // Footer
    ctx.fillStyle = `${THEME.primary}80`
    ctx.font = '10px "Segoe UI"'
    ctx.fillText('❥ Rimuru AI', W - 70, H - 8)
...
async function handler(m, { sock, config: botConfig }) {
    try {
        ...
        if (sub === 'resign' || sub === 'nyerah') {
            ...
            if (!game) return m.reply('❌ لا توجد لعبة ضامة جارية يا عزيزي~')
            if (game.p1 !== senderId && game.p2 !== senderId)
                return m.reply('❌ عزيزي، أنت لا تلعب هنا~')
            ...
            return m.reply(
                ...
                `🏆 @${winner.split('@')[0]} *فاز!*\n\n` +
                `🦋 *رمورو:* يا لهاي~ لا تستسلم في المرة القادمة! 💕`
            )
        }
        ...
        if (args.length >= 2 && sessions.has(chatId)) {
            ...
            if (senderId !== currentPlayer) {
                return m.reply(`⏳ ليس دورك يا عزيزي!\nانتظر @${currentPlayer.split('@')[0]} ليتحرك أولاً~ 🦋`)
            }
            ...
            if (!fromPos || !toPos) {
                return m.reply(`❌ صيغة خاطئة!\nمثال: *${prefix}checkers A3 B4*\n\nالأعمدة: A-H | الصفوف: 1-8`)
            }
            ...
            if (!piece || (piece !== playerPiece && piece !== `${playerPiece}_king`)) {
                return m.reply(`❌ لا تملك قطعة في ${args[0].toUpperCase()}!`)
            }
            ...
            if (hasCapture) {
                ...
                if (Math.abs(toPos.row - fromPos.row) !== 2 || Math.abs(toPos.col - fromPos.col) !== 2) {
                    return m.reply(`❌ يجب عليك أكل قطعة الخصم! حرّك قفزةً مزدوجة قطرية للالتقاط.`)
                }
                
                if (!canCapture(game.board, fromPos.row, fromPos.col, toPos.row, toPos.col, playerPiece)) {
                    return m.reply(`❌ التقاط غير صالح! تأكد من وجود قطعة خصم في المنتصف.`)
                }
            } else {
                // Gerak biasa
                if (!canMove(game.board, fromPos.row, fromPos.col, toPos.row, toPos.col, playerPiece)) {
                    return m.reply(`❌ حركة غير صالحة! القطعة تتحرك خطوة قطرية واحدة فقط.`)
                }
            }
            ...
            // التحقق من وجود التقاط إضافي من الموضع الجديد
            ...
            if (isCapture) {
                ...
                if (newPiece === playerPiece || newPiece === `${playerPiece}_king`) {
                    // التحقق من إمكانية الالتقاط مجدداً من الموضع الجديد
                    for (let dr of [-2, 2]) {
                        for (let dc of [-2, 2]) {
            ...
            if (p1Count === 0 || p2Count === 0 || !hasAnyCapture(game.board, 'p1') || !hasAnyCapture(game.board, 'p2')) {
                ...
                if (winner) {
                    ...
                    return sock.sendMessage(chatId, {
                        image: img,
                        caption: `🎉 *انتهت اللعبة!* 🎉\n\n🏆 @${winner.split('@')[0]} *فاز!*\n\n📊 النتيجة النهائية: ${p1Count} مقابل ${p2Count}\n\n🦋 *رمورو:* مبروك يا عزيزي~ ${winner.split('@')[0]} ذكي جداً! 💕`,
                        mentions: [winner]
                    }, { quoted: m })
                }
            }
            
            // تبديل الدور أو متابعة الالتقاط
            if (mustCaptureAgain) {
                game.mustCapture = true
                game.captureFrom = captureFrom
                // يبقى الدور نفسه لأن الالتقاط ما زال ممكناً
            } else {
            ...
            if (mustCaptureAgain) caption += `✨ *متابعة!* ما زلت تستطيع الالتقاط! حرّك القطعة نفسها.\n`
        ...
        if (!sessions.has(chatId)) {
            ...
            if (!opponent || opponent === senderId) {
                return m.reply(
                    ...
                    `• يمكنك "أكل" قطعة الخصم بالقفز خطوتين\n` +
                    `• Kalo sampai ujung seberang, jadi RAJA (👑) bisa gerak mundur\n` +
                    `• الهدف: التهم كل قطع الخصم!\n\n` +
                    `📌 *Format gerak:*\n` +
                    `• Kolom: A-H | Baris: 1-8\n` +
                    `• Contoh: ${prefix}checkers A3 B4\n\n` +
                    `💕 *البدء:* ${prefix}checkers @الخصم\n` +
                    `🏳️ *الاستسلام:* ${prefix}checkers resign`
                )
            }
            ...
            const sentMsg = await sock.sendMessage(chatId, {
                image: img,
                caption: `🎲 *RIMURU CHECKERS DIMULAI!* 🎲\n\n` +
                    `🔴 *Player 1:* @${senderId.split('@')[0]}\n` +
                    `💕 *Player 2:* @${opponent.split('@')[0]}\n\n` +
                    `🎮 *الدور الأول:* @${firstPlayer.split('@')[0]}\n\n` +
                    `🦋 *رمورو:* ${['هيا نلعب الضامة يا عزيزي~', 'من الخبير في الاستراتيجية هنا؟', 'لا تنس الترقية إلى ملك~'][Math.floor(Math.random() * 3)]} 💕\n\n` +
        ...
        return sock.sendMessage(m.chat, {
            ...
            caption: `🎲 *CHECKERS ACTIVE*\n\n` +
                ...
                `🎮 الدور @${currentPlayer.split('@')[0]}\n\n` +
                `📌 ${prefix}checkers A3 B4 - للتحرك\n` +
                `🏳️ ${prefix}checkers resign - استسلام`,
            mentions: [currentPlayer]
        }, { quoted: m })
        
    } catch (err) {
        console.error('[CHECKERS] ERROR:', err)
        await m.reply(`❌ حدث خطأ: ${err.message}\n\nحاول مجدداً يا عزيزي~ 🦋`)
    }
}
