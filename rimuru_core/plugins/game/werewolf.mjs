...
try {
  const assetsPath = path.join(RIMURU_CORE_ROOT, "assets", "images");
  if (fs.existsSync(path.join(assetsPath, "rimuru-games.jpg"))) {
    thumbWW = fs.readFileSync(path.join(assetsPath, "rimuru-games.jpg"));
  }
  if (fs.existsSync(path.join(assetsPath, "rimuru.jpg"))) {
    thumbNight = fs.readFileSync(path.join(assetsPath, "rimuru.jpg"));
    thumbDay = fs.readFileSync(path.join(assetsPath, "rimuru.jpg"));
  }
  if (fs.existsSync(path.join(assetsPath, "rimuru-winner.jpg"))) {
    thumbWin = fs.readFileSync(path.join(assetsPath, "rimuru-winner.jpg"));
  }
} catch (e) {
...
const ROLES = {
  werewolf: {
    ...
    name: "Werewolf",
    team: "wolf",
    desc: "اقتل مواطناً كل ليلة",
  },
  seer: {
    emoji: "🔮",
    name: "Seer",
    team: "village",
    desc: "اطّلع على دور اللاعبين كل ليلة",
  },
  guardian: {
    emoji: "🛡️",
    name: "Guardian",
    team: "village",
    desc: "احمِ لاعباً كل ليلة",
  },
  sorcerer: {
    emoji: "🧙",
    name: "Sorcerer",
    team: "wolf",
    desc: "اكتشف من هو العرّاف",
  },
  villager: {
    emoji: "👨‍🌾",
    name: "Villager",
    team: "village",
    desc: "ناقش وصوّت ضد المستذئب",
  },
};
...
async function sendWW(sock, jid, text, title, body, thumbBuffer, mentions) {
  const msgId = await sock.sendPreview(
    jid,
    {
      ...
      description: body || "لعبة استنتاج اجتماعي!",
...
function getRoleDescription(role, prefix = ".") {
  const descriptions = {
    ...
    seer:
      ...
      `┃ 🔮 المهارة: اطّلع على دور لاعب واحد\n` +
      `┃ 🕐 Aksi: Malam hari\n` +
      `╰┈┈┈┈┈┈┈┈⬡\n\n` +
      `> في الليل، اكتب:\n` +
...
async function handler(m, { sock }) {
  ...
  const commands = {
    create: async () => {
        ...
        if (game.status === "waiting") {
          return m.reply(
            `❌ *الغرفة موجودة بالفعل*\n\n` +
              ...
              `اكتب \`${prefix}ww join\` للانضمام\n` +
              `Host: @${game.owner.split("@")[0]}`,
            { mentions: [game.owner] },
          );
        }
        return m.reply(`❌ اللعبة جارية! انتظر حتى تنتهي.`);
      }
      ...
      if (existingRoom) {
        return m.reply(`❌ أنت في لعبة جارية في مجموعة أخرى!`);
      }
    ...
    join: async () => {
      if (!ww[m.chat]) {
        return m.reply(
          `❌ لا توجد غرفة!\n> اكتب \`${prefix}ww create\` لإنشاء غرفة`,
        );
      }

      if (ww[m.chat].status !== "waiting") {
        return m.reply(`❌ بدأت اللعبة بالفعل! انتظر الجولة التالية.`);
      }
      ...
      if (ww[m.chat].players.some((p) => p.id === m.sender)) {
        return m.reply(`❌ أنت منضم بالفعل!`);
      }
      ...
      await m.reply(
        `✅ *PLAYER BERGABUNG*\n\n` +
          ...
          `${playerList
            ...
            .join("\n")}\n` +
          `╰┈┈┈┈┈┈┈┈⬡\n\n` +
          `الإجمالي: ${ww[m.chat].players.length}/${MIN_PLAYERS} (الحد الأدنى)\n` +
          (canStart
            ? `✅ يمكن البدء! \`${prefix}ww start\``
            : `🕕 يلزم ${MIN_PLAYERS - ww[m.chat].players.length} لاعبين آخرين`),
    ...
    start: async () => {
      if (!ww[m.chat]) {
        return m.reply(`❌ لا توجد غرفة!`);
      }

      if (ww[m.chat].status !== "waiting") {
        return m.reply(`❌ اللعبة جارية بالفعل!`);
      }

      if (ww[m.chat].owner !== m.sender && !config.isOwner?.(m.sender)) {
        return m.reply(`❌ فقط المضيف يمكنه بدء اللعبة!`);
      }
      ...
      await m.reply(
        `🐺 *GAME DIMULAI!*\n\n` +
          ...
          `${roleInfo
            ...
            .join("\n")}\n` +
          `╰┈┈┈┈┈┈┈┈⬡\n\n` +
          `📩 تحقق من رسائلك الخاصة لمعرفة أدواركم!\n` +
          `🌙 Werewolf berburu...\n` +
          `⏱️ مدة الليل: ${PHASE_DURATION.night / 1000} ثانية`,
        { mentions: ww[m.chat].players.map((p) => p.id) },
      );
    ...
    vote: async () => {
      ...
      if (ww[m.chat].phase !== "day") {
        return m.reply(
          `❌ ليس وقت التصويت الآن!\n> المرحلة: ${ww[m.chat].phase === "night" ? "🌙 ليل" : ww[m.chat].phase}`,
        );
      }

      const player = ww[m.chat].players.find((p) => p.id === m.sender);
      if (!player) {
        return m.reply(`❌ أنت لست لاعباً في هذه اللعبة!`);
      }

      if (!player.alive) {
        return m.reply(`❌ أنت ميت! لا يمكنك التصويت.`);
      }

      if (player.voted) {
        return m.reply(`❌ صوّت بالفعل! انتظر نتيجة التصويت.`);
      }
      ...
      if (!targetPlayer) {
        return m.reply(`❌ اللاعب رقم ${targetNum} غير موجود!`);
      }

      if (!targetPlayer.alive) {
        return m.reply(`❌ هذا اللاعب ميت بالفعل!`);
      }
    ...
    player: async () => {
      if (!ww[m.chat]) {
        return m.reply(`❌ لا توجد لعبة في هذه الغرفة!`);
      }
    ...
    exit: async () => {
      ...
      if (playerIdx === -1) {
        return m.reply(`❌ أنت لست في هذه اللعبة!`);
      }

      if (ww[m.chat].status === "playing") {
        return m.reply(`❌ لا يمكن الخروج أثناء اللعب!`);
      }
    ...
    delete: async () => {
      ...
      if (!isOwner && !isBotOwner) {
        return m.reply(`❌ فقط المضيف أو مالك البوت يمكنه الحذف!`);
      }
  ...
  if (!action || !commands[action]) {
    return m.reply(
      `🐺 *WEREWOLF GAME*\n\n` +
        ...
        `┃ 🧙 الساحر - ابحث عن العرّاف\n` +
        `┃ 🔮 العرّاف - اطّلع على الأدوار\n` +
        `┃ 🛡️ Guardian - Lindungi\n` +
    ...
    try {
      ...
      switch (player.role) {
        case "werewolf":
          text =
            `🐺 *MALAM HARI*\n\n` +
            `حان وقت الصيد! اختر هدفك:\n\n` +
            `${playerListWolf}\n` +
            `> اكتب \`${prefix}wwkill <رقم>\` للقتل`;
          break;
        case "seer":
          text =
            `🔮 *MALAM HARI*\n\n` +
            `من تريد الاطلاع على دوره؟\n\n` +
            `${playerListNormal}\n` +
            `> اكتب \`${prefix}wwsee <رقم>\` للاطلاع على الدور`;
          break;
        case "guardian":
          text =
            `🛡️ *MALAM HARI*\n\n` +
            `من تريد حمايته؟\n\n` +
            `${playerListNormal}\n` +
            `> اكتب \`${prefix}wwprotect <رقم>\` للحماية`;
          break;
        case "sorcerer":
          text =
            `🧙 *MALAM HARI*\n\n` +
            `اكتشف من هو العرّاف!\n\n` +
            `${playerListWolf}\n` +
            `> اكتب \`${prefix}wwsorcerer <رقم>\` للتحقق`;
          break;
        case "villager":
          text =
            `👨‍🌾 *MALAM HARI*\n\n` +
            `Sebagai warga, berhati-hatilah.\n` +
            `ربما أنت الهدف التالي.\n\n` +
            `${playerListNormal}`;
          break;
      }
...
async function processNightActions(chatId, sock, db, prefix) {
  ...
  if (killTarget && killTarget !== protectTarget) {
    ...
    nightReport += `🛡️ نجح الحارس في حماية الهدف!\n`;
    nightReport += `> لا ضحايا هذه الليلة.\n\n`;
  } else {
    nightReport += `🌅 ليلة هادئة...\n`;
    nightReport += `> لا ضحايا.\n\n`;
  }
  ...
  nightReport += `${playerList
    ...
    .join("\n")}\n`;
  nightReport += `╰┈┈┈┈┈┈┈┈⬡\n\n`;
  nightReport += `> 🗳️ حان وقت التصويت!\n`;
  nightReport += `> اكتب \`${prefix}ww vote <رقم>\`\n`;
  nightReport += `> ⏱️ المدة: ${PHASE_DURATION.day / 1000} ثانية`;
...
async function executeVote(chatId, sock, db, prefix) {
  ...
  if (isTie || maxVotes === 0) {
    resultText += `🤷 لم يُقصَ أحد!\n`;
    resultText += `> ${isTie ? "تصويت متعادل!" : "لم يصوّت أحد."}\n\n`;
  } else if (eliminated) {
  ...
  resultText += `> أصحاب الأدوار الخاصة، استخدموا مهاراتكم في الرسائل الخاصة!\n`;
  resultText += `> ⏱️ المدة: ${PHASE_DURATION.night / 1000} ثانية`;
...
async function endGame(chatId, sock, db, winner) {
  ...
  const endText =
    `🎉 *GAME OVER!*\n\n` +
    `${winner === "wolf" ? "🐺 *فاز المستذئب!*" : "👨‍🌾 *فاز القرويون!*"}\n\n` +
    `╭┈┈⬡「 👥 *كل اللاعبين* 」\n` +
    `${allPlayers
      .split("\n")
      .map((l) => `┃ ${l}`)
    ...
    `╰┈┈┈┈┈┈┈┈⬡\n\n` +
    `╭┈┈⬡「 🎁 *الجائزة* 」\n` +
    `┃ 💰 +${WIN_REWARD.koin.toLocaleString()} عملة\n` +
    `┃ ⭐ +${WIN_REWARD.exp.toLocaleString()} EXP\n` +
    `╰┈┈┈┈┈┈┈┈⬡\n\n` +
    `> أحسنتم! تلعب مرة أخرى؟ \`${config.command?.prefix || "."}ww create\``;
...
async function nightActionHandler(m, { sock }) {
  ...
  if (!chatId) {
    return m.reply(
      `❌ أنت لست في لعبة مستذئب أو ليست مرحلة الليل!`,
    );
  }
  ...
  if (!player || !player.alive) {
    return m.reply(`❌ أنت ميت أو لست لاعباً!`);
  }

  // Check if skill already used
  if (player.skillUsed) {
    return m.reply(`❌ استخدمت مهارتك لهذه الليلة!`);
  }
  ...
  if (!targetPlayer) {
    return m.reply(`❌ هدف غير صالح أو ميت بالفعل!`);
  }
  ...
  if (cmd === "wwkill" && player.role === "werewolf") {
    if (targetPlayer.role === "werewolf" || targetPlayer.role === "sorcerer") {
      return m.reply(`❌ لا يمكنك قتل زميل في الفريق!`);
    }
  ...
  return m.reply(
    `❌ لا تملك هذه المهارة!\n> دورك: ${ROLES[player.role]?.name || "مجهول"}`,
  );
}
