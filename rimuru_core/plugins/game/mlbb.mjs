...
const mlbb = {
    ...
    counterPick: async (enemyNames = []) => {
        ...
        if (enemyIds.length === 0) return { success: false, message: "بطل الخصم غير موجود." };
    ...
    synergy: async (allyNames = []) => {
        ...
        if (allyIds.length === 0) return { success: false, message: "بطل الفريق غير موجود." };
    ...
    matchup: async (teamANames = [], teamBNames = []) => {
        ...
        if (teamAIds.length === 0 || teamBIds.length === 0) {
            return { success: false, message: "بطل الفريق A أو B غير صالح." };
        }
...
async function handler(m, { text, command, prefix }) {
    ...
    if (!text && command === 'mlbb') {
        ...
        return m.reply(
            ...
            `استخدم الأوامر الفرعية التالية:\n` +
    ...
    try {
        ...
        if (subCommand === 'counter') {
            ...
            if (!res || res.success === false) {
                if (m.react) await m.react('❌');
                return m.reply(`❌ فشل في الحصول على الكاونتر: ${res?.message || 'البطل غير موجود.'}`);
            }
            ...
            if (Array.isArray(suggestions) && suggestions.length > 0) {
                ...
                txt += `> بيانات الكاونتر غير موجودة.`;
            }
        ...
        // 2. SYNERGY COMBO
            ...
            if (!res || res.success === false) {
                if (m.react) await m.react('❌');
                return m.reply(`❌ فشل في الحصول على التآزر: ${res?.message || 'البطل غير موجود.'}`);
            }
        ...
        // 3. MATCHUP PREDICTION
            ...
            if (!inputParam.includes('|')) {
                return m.reply(`❌ افصل الفريق A والفريق B بعلامة \`|\`!\n*مثال:* \`${pfx}mlbb matchup fanny, angela | saber, tigreal\``);
            }
            ...
            if (!res || res.success === false) {
                if (m.react) await m.react('❌');
                return m.reply(`❌ فشل في توقع المواجهة: ${res?.message || 'بيانات غير صالحة.'}`);
            }
        ...
        // 4. HERO TIERS
            ...
            if (!res) {
                if (m.react) await m.react('❌');
                return m.reply('❌ فشل في جلب بيانات تصنيفات الأبطال.');
            }
            ...
            if (m.react) await m.react('❌');
            return m.reply(`❌ أمر فرعي غير معروف.`);
        }
