...
import {
  ...
  healPlayer,
  JACKPOT_POOLS,
} from "../../src/lib/rimuru-minecraft.mjs";
import {
  biomes,
  travelRequirements,
  pickaxes,
  ...
  SMELT_RECIPES,
  CRAFT_RECIPES,
} from "../../src/lib/rimuru-minecraft-data.mjs";
...
let thumbMC = null;
try {
  const p = path.join(RIMURU_CORE_ROOT, "assets", "images", "rimuru-minecraft.jpg");
  if (fs.existsSync(p)) thumbMC = fs.readFileSync(p);
} catch (e) {}

function ctx() {
  const sId = RIMURU_CORE_CONFIG.saluran?.id || "120363400911374213@newsletter";
  const sName = RIMURU_CORE_CONFIG.saluran?.name || RIMURU_CORE_CONFIG.bot?.name || "rimuru-AI";
  return {
...
async function handler(m, { sock }) {
    ...
    if (!sub || sub === "on" || sub === "off") {
      ...
      if (sub === "on") {
        ...
        return send(
          sock,
          m,
          `*🧱 تم تفعيل ماينكرافت*\n\n🎮 على جميع الأعضاء لعب ماينكرافت!\n🪧 اكتب \`.mct help\` للبدء`,
          "🧱 Minecraft ON",
          "✅ Aktif",
        );
      }
  ...
  if (!sub || sub === "help" || sub === "menu") {
    return send(
      ...
      `*🧱 MINECRAFT GAME*\n_⛏️ Sistem pertambangan & 🛠️ crafting terlengkap_\n\n` +
        `*⛏️ التعدين*\n\`.mct mine\` _⛏️ ابدأ التعدين_\n\`.mct collect\` _📦 اجمع الناتج_\n\`.mct sell\` _💸 بِع الخامات_\n\`.mct orebook\` _📚 مجموعة الخامات_\n\`.mct top\` _🏆 قائمة الترتيب_\n\n` +
        ...
        `*🔥 التصنيع والصهر*\n\`.mct smelt\` _🔥 صهر كل الخامات_\n\`.mct craft\` _🛠️ قائمة الوصفات_\n\`.mct craft <id>\` _🧪 اصنع عنصراً_\n\`.mct inv\` _🎒 الحقيبة_\n\n` +
  ...
  if (sub === "mine") {
    ...
    if (mc.miningPending && mc.miningPending.length > 0)
      return m.reply(`_📦 لا يزال هناك ناتج تعدين!_ \`.mct collect\` _أولاً._`);
    const now = Date.now();
    if (mc.lastMineTime && now - mc.lastMineTime < MC * 1000)
      return m.reply(
        `_⏳ Tunggu *${Math.ceil((MC * 1000 - (now - mc.lastMineTime)) / 1000)}* ثانية_`,
      );
    const pk = mc.usedPickaxe || "woodpick";
    const pick = mc.pickaxes[pk];
    if (!pick) return m.reply(`_🪓 لا يوجد معول نشط!_ \`.mct picks\``);
  ...
  if (sub === "collect") {
    ...
    const mc = user.minecraft;
    if (!mc.miningPending || mc.miningPending.length === 0)
      return m.reply(`_📭 لا يوجد ناتج._ \`.mct mine\` _أولاً!_`);
  ...
  if (sub === "sell") {
    ...
    return send(
      sock,
      m,
      `*💸 تم بيع الخامات!*\n\n📦 الكمية: ${fc2}\n💰 الإجمالي: ${formatMoney(fv)}\n🏦 الرصيد: ${formatMoney(mc.money)}`,
      "💸 Ore Terjual!",
      `💰 ${formatMoney(fv)}`,
    );
  }
  ...
  if (sub === "travel") {
    ...
    if (!biomes[tk]) return m.reply(`_🗺️ المنطقة غير موجودة!_ \`.mct travel\``);
    if (mc.currentBiome === tk)
      return m.reply(`_📍 أنت في ${biomes[tk].name} بالفعل!_`);
    const req = travelRequirements[tk];
    if (req) {
      if ((mc.money || 0) < req.money)
        return m.reply(`_💸 المال غير كافٍ! تحتاج ${formatMoney(req.money)}_`);
  ...
  if (sub === "buy") {
    ...
    if (!pickaxes[pk]) return m.reply(`_🪓 المعول غير موجود!_ \`.mct shop\``);
    if (mc.pickaxes[pk])
      return m.reply(`_✅ تملك ${pickaxes[pk].name} بالفعل!_`);
    if (pickaxes[pk].price === 0)
      return m.reply(`_🪙 Pickaxe ini dari Token/Prestige!_`);
    if ((mc.money || 0) < pickaxes[pk].price)
      return m.reply(
        `_💸 المال غير كافٍ! تحتاج ${formatMoney(pickaxes[pk].price)}_`,
      );
  ...
  if (sub === "equip") {
    ...
    if (!mc.pickaxes[pk])
      return m.reply(`_📭 لا تملك هذا المعول!_ \`.mct picks\``);
    mc.usedPickaxe = pk;
    db.markDirty("users");
    return send(
      sock,
  ...
  if (sub === "picks") {
    ...
    if (Object.keys(pks).length === 0)
      return m.reply(`_📭 لا تملك معولاً!_`);
  ...
  if (sub === "enchant") {
    ...
    const pk = mc.usedPickaxe || "woodpick";
    const pick = mc.pickaxes[pk];
    if (!pick) return m.reply(`_🪓 لا يوجد معول نشط!_`);
    const ek = sa[0] ? sa[0].toLowerCase() : "";
    if (!ek) {
      if (pick.enchant) {
        const e = pickEnchants[pick.enchant];
        return m.reply(
          `_✨ التسليح الحالي: *${e ? e.name : pick.enchant}* (${e ? e.rarity : "?"})_\n\`.mct enchant <key>\` للتغيير`,
        );
      }
      return m.reply(`_✨ Tentukan enchant!_ \`.mct enchants\``);
    }
    if (!pickEnchants[ek])
      return m.reply(`_✨ التسليح غير موجود!_ \`.mct enchants\``);
    const ench = pickEnchants[ek];
    const cost = encCost(ench.rarity);
    if ((mc.money || 0) < cost)
      return m.reply(`_💸 المال غير كافٍ! تحتاج ${formatMoney(cost)}_`);
    mc.money -= cost;
    pick.enchant = ek;
    db.markDirty("users");
    return send(
  ...
  if (sub === "pickup") {
    ...
    if (pick.level >= pick.maxLevel)
      return m.reply(`_⬆️ المعول بلغ أقصى مستوى!_`);
    const cost =
  ...
  if (sub === "fight") {
    ...
    if (result.won) {
      ...
      return send(sock, m, txt, "🏆 فوز!", result.mobName);
    } else {
      let txt = `*💀 لقد خسرت!*\n\n⚔️ ضد *${result.mobName}*\n\n`;
      for (const line of result.log) txt += `${line}\n`;
      txt += `\n_❤️ الصحة المتبقية: ${mc.hp}/${mc.maxHp}_\n\`.mct heal\` لـ ❤️ التعافي`;
      return send(sock, m, txt, "💀 خسارة!", result.mobName);
    }
  }
  ...
  if (sub === "smelt") {
    ...
    if (result.count === 0)
      return m.reply(`_🔥 لا توجد خامات قابلة للصهر!_`);
    ...
    txt += `\n*💰 الإجمالي: ${formatMoney(result.totalValue)}*\n*🏦 الرصيد: ${formatMoney(mc.money)}*`;
    db.markDirty("users");
    return send(
      sock,
      m,
  ...
  if (sub === "craft") {
    ...
    return send(
      sock,
      m,
      `*🛠️ نجح التصنيع!*\n\n*${result.item}*\n_💰 القيمة: ${formatMoney(result.value)}_`,
      "🛠️ Crafted!",
      result.item,
    );
  }
  ...
  if (sub === "daily") {
    ...
    if (mc.lastDaily) {
      ...
      if (diff < 86400000)
        return m.reply(
          `_🎁 تم استلام المكافأة اليومية! انتظر *${Math.ceil((86400000 - diff) / 3600000)}* ساعة._`,
        );
    }
    ...
    return send(
      sock,
      m,
      `*🎁 المكافأة اليومية!*\n\n*🔥 التتابع:* ${mc.dailyStreak} يوم\n💰 +${formatMoney(rw.money)}\n🎟️ +${rw.tickets} تذكرة غاتشا\n*🏦 الرصيد:* ${formatMoney(mc.money)}`,
      "🎁 Daily Reward!",
  ...
  if (sub === "gacha") {
    ...
    if (useT) {
      ...
      if ((mc.money || 0) < GACHA_COST_COINS)
        return m.reply(
          `_💸 المال غير كافٍ! تحتاج ${formatMoney(GACHA_COST_COINS)}_`,
        );
    ...
    if (result.isSSR) txt += `\n*🌈 SSR PULL!*`;
    if (result.pity) txt += `\n*🔥 Pity Activated!*`;
    txt += `\n\n_🔥 الضمان: ${mc.gachaPity}/${GACHA_PITY_LIMIT}_\n_🏦 الرصيد: ${formatMoney(mc.money)} | 🎟️ التذاكر: ${mc.gachaTickets}_`;
    db.markDirty("users");
    return send(
      sock,
      m,
  ...
  if (sub === "upgrade") {
    ...
    if (lv >= upg.maxLevel) return m.reply(`_✅ بلغ الحد الأقصى بالفعل!_`);
  ...
  if (sub === "prestige") {
    ...
    if (sa[0] && sa[0].toLowerCase() === "confirm") {
      ...
      const req = reqs[cp];
      if (!req) return m.reply(`_👑 بلغت أقصى هيبة (برستيج)!_`);
      if ((mc.blocksMined || 0) < req.blocks)
        return m.reply(`_🧱 Blocks kurang! Butuh ${req.blocks}_`);
      if ((mc.money || 0) < req.money)
        return m.reply(`_💸 المال غير كافٍ! تحتاج ${formatMoney(req.money)}_`);
      ...
      return send(
        sock,
        m,
        `*👑 ترقية هيبة!*\n\n*🏷️ اللقب:* ${titles[mc.prestige]}\n*🪙 الرموز:* ${mc.prestigeTokens}\n\n_💸 المال -90%، و🧱 عدّاد المكعبات أُعيد ضبطه_`,
        "👑 PRESTIGE!",
        titles[mc.prestige],
      );
    }
  ...
  if (sub === "tokens") {
    ...
    if (!item) return m.reply(`_🛒 العنصر غير موجود!_ \`.mct tokens\``);
    ...
    switch (item.type) {
      case "pickaxe":
        if (pickaxes[item.value] && !mc.pickaxes[item.value]) {
          ...
          mc.prestigeTokens += item.cost;
          return m.reply(`_✅ تملك هذا المعول بالفعل!_`);
        }
        break;
      case "tickets":
        mc.gachaTickets = (mc.gachaTickets || 0) + item.value;
  ...
  if (sub === "jackpot") {
    ...
    const mc = user.minecraft;
    if (!sa[0]) {
      let txt = `*🎰 نظام الجائزة الكبرى*\n\n_💎 نظام جائزة كبرى بمكافآت ضخمة!_\n_🎁 يمكن أن تحصل على _بريميوم_ و_شريك_ و_طاقة_ و_حد يومي_، وحتى _غير محدود_!_\n\n`;
    ...
    if (!pool) return m.reply(`_🎰 الفئة غير موجودة!_ \`.mct jackpot\``);
    if ((mc.money || 0) < pool.cost)
      return m.reply(`_💸 المال غير كافٍ! تحتاج ${formatMoney(pool.cost)}_`);
    mc.money -= pool.cost;
    const result = doJackpotPull(mc, poolId);
    if (!result) return m.reply(`_❌ فشل! حاول مجدداً._`);
  ...
  if (sub === "top") {
    ...
    if (rankings.length === 0) return m.reply(`_👤 لا يوجد لاعبون بعد!_`);
    rankings.sort((a, b) => {
    ...
    return send(sock, m, txt.trim(), "🏆 قائمة الترتيب", "👑 أفضل اللاعبين");
  }
}
