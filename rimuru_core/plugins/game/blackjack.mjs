        ...
        switch (aksi) {
            case 'end':
                if (conn.blackjack[m.chat]?.idPemain === m.sender) {
                    ...
                    await conn.reply(m.chat, '*لا توجد جلسة بلاك جاك جارية، أو أنك لست لاعباً فيها.*', m);
                }
                break;
            ...
            case 'hit':
                if (!conn.blackjack[m.chat] || conn.blackjack[m.chat]?.idPemain !== m.sender) {
                    await conn.reply(m.chat, '*أنت لا تلعب البلاك جاك الآن، أو أنك لست لاعباً فيها.*', m);
                    break;
                }
            ...
            default:
                await conn.reply(m.chat, `*أمر غير صالح.*\nاستخدم *${usedPrefix + command} start* لبدء جلسة البلاك جاك.`, m);
                break;
        }
    } catch (err) {
        console.error(err);
