...
<div class="hint">اقفز عندما تومض خطوط الليزر الحمراء بسرعة! · لمس العدو = خسارة 10% من الحلقات · كل 50 حلقة = زعيم! · 🔊 = صوت</div>
...
/* الحلقات: تُفقد 10% فقط (1 كحد أدنى، 14 بصرياً) */
...
/* حلقة منفلتة (يمكن التقاطها مجدداً) */
...
/* البرج + التسديد + الشعاع (فوق الجميع للوضوح) */
drawTurret();
drawAim();
drawBeam();
/* partikel */
...
x.font='900 15px Arial';x.fillStyle='rgba(255,255,255,'+(.55+.45*Math.sin(frame*.09))+')';x.fillText('المس للبدء',W/2,178);
...
x.font='700 11px Arial';x.fillStyle='rgba(255,255,255,'+(.5+.5*Math.sin(frame*.1))+')';x.fillText('المس للعب مجدداً',W/2,188);
x.textAlign='left';
}
}
...
async function handler(m, { sock }) {
  try {
    ...
    console.error('[SONIK]', e?.message || e)
