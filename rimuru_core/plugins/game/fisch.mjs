...
import {
  ...
  GACHA_COST_COINS,
  GACHA_PITY_LIMIT,
} from "../../src/lib/rimuru-fisch-data.mjs";
...
let thumbFish = null;
try {
  const p = path.join(RIMURU_CORE_ROOT, "assets", "images", "rimuru-fishit.jpg");
...
function send(sock, m, text, title, body) {
  const msgId = sock.sendPreview(
    m.chat,
    {
      ...
      title: `𝗙𝗜𝗦𝗛 𝗜𝗧 𝗚𝗔𝗠𝗘𝗦`,
      description: `احصل على الجوائز ومتعتها من صيدك`,
...
async function handler(m, { sock }) {
    ...
    if (!sub || sub === "on" || sub === "off") {
      ...
      if (sub === "on") {
        ...
        return send(
          sock,
          m,
          `*تم تفعيل FISHIT*\n\nعلى جميع الأعضاء لعب Fishit!\nاكتب \`.fisht help\` للبدء`,
          "Fishit ON",
          "Aktifkan",
        );
      }
  ...
  if (sub === "mancing" || sub === "fish") {
    ...
    if (f.fishingPending && f.fishingPending.length > 0)
      return m.reply(`_لا يزال لديك صيد!_ \`.fisht view\` _أولاً._`);
    const now = Date.now();
    if (f.lastFishTime && now - f.lastFishTime < FC * 1000)
      return m.reply(
        `_Tunggu *${Math.ceil((FC * 1000 - (now - f.lastFishTime)) / 1000)}* ثانية_`,
      );
    const rk = f.usedFishingRod || "basicrod";
    const rod = f.fishingRods[rk];
    if (!rod) return m.reply(`_لا يوجد قضيب صيد نشط!_ \`.fisht rods\``);
  ...
  if (sub === "view") {
    ...
    const f = user.fisch;
    if (!f.fishingPending || f.fishingPending.length === 0)
      return m.reply(`_لا يوجد صيد._ \`.fisht mancing\` _أولاً!_`);
  ...
  if (sub === "travel") {
    ...
    if (!islands[tk]) return m.reply(`_الجزيرة غير موجودة!_ \`.fisht travel\``);
  ...
  if (sub === "buy") {
    ...
    if (!fishingRod[rk]) return m.reply(`_القضيب غير موجود!_ \`.fisht shop\``);
    if (f.fishingRods[rk])
  ...
  if (sub === "enchant") {
    ...
    const rk = f.usedFishingRod || "basicrod";
    const rod = f.fishingRods[rk];
    if (!rod) return m.reply(`_لا يوجد قضيب صيد نشط!_`);
    const ek = sa[0] ? sa[0].toLowerCase() : "";
    if (!ek) {
      if (rod.enchant) {
        const e = rodEnchants[rod.enchant];
        return m.reply(
          `_التسليح الحالي: *${e ? e.name : rod.enchant}* (${e ? e.rarity : "?"})_\n\`.fisht enchant <key>\` للتغيير`,
        );
      }
      return m.reply(`_Tentukan enchant!_ \`.fisht enchants\``);
    }
    if (!rodEnchants[ek])
      return m.reply(`_التسليح غير موجود!_ \`.fisht enchants\``);
  ...
  if (sub === "rodup") {
    ...
    const rod = f.fishingRods[rk];
    if (!rod) return m.reply(`_لا يوجد قضيب صيد نشط!_`);
    if (rod.level >= rod.maxLevel) return m.reply(`_القضيب بلغ أقصى مستوى!_`);
  ...
  if (sub === "daily") {
    ...
    if (f.lastDaily) {
      ...
      if (diff < 86400000)
        return m.reply(
          `_تم استلام المكافأة اليومية! انتظر *${Math.ceil((86400000 - diff) / 3600000)}* ساعة._`,
        );
    }
    ...
    return send(
      sock,
      m,
      `*المكافأة اليومية!*\n\n*التتابع:* ${f.dailyStreak} يوم\n+${formatMoney(rw.money)}\n+${rw.tickets} تذكرة غاتشا\n*الرصيد:* ${formatMoney(f.money)}`,
      "Daily Reward!",
  ...
  if (sub === "tokens") {
    ...
    if (!item) return m.reply(`_العنصر غير موجود!_ \`.fisht tokens\``);
  ...
  if (sub === "jackpot") {
    ...
    const f = user.fisch;
    if (!sa[0]) {
      let txt = `*نظام الجائزة الكبرى*\n\n_نظام جائزة كبرى بمكافآت ضخمة!_\n_يمكن أن تحصل على _بريميوم_ و_شريك_ و_طاقة_ و_حد يومي_، وحتى _غير محدود_!_\n\n`;
    ...
    if (!result) return m.reply(`_فشل! حاول مجدداً._`);
  ...
  if (sub === "top") {
    ...
    if (rankings.length === 0) return m.reply(`_لا يوجد لاعبون بعد!_`);
    rankings.sort((a, b) => {
    ...
    return send(sock, m, txt.trim(), "Leaderboard", "أفضل اللاعبين");
  }
}
