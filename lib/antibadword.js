...
function loadAntibadwordConfig(groupId, sock) {
    try {
        const sid = sock && sock._sessionNumber ? sock._sessionNumber : null;
        const configPath = sid
            ? path.join(__dirname, '../data/userGroupData_' + sid + '.json')
...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    ...
    if (match.startsWith('set')) {
        ...
        await setAntiBadword(chatId, 'on', action, sock._sessionNumber || null);
...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    const config = loadAntibadwordConfig(chatId, sock);
    ...
    const antiBadwordConfig = await getAntiBadword(chatId, 'on', sock._sessionNumber || null);
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    switch (antiBadwordConfig.action) {
        ...
        case 'warn':
            const warningCount = await incrementWarningCount(chatId, senderId, sock._sessionNumber || null);
            if (warningCount >= 3) {
                try {
                    await sock.groupParticipantsUpdate(chatId, [senderId], 'remove');
                    await resetWarningCount(chatId, senderId, sock._sessionNumber || null);
                    await sock.sendMessage(chatId, {
