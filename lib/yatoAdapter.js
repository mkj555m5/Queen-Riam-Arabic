...
// تحميل media من رسالة (لدعم m.quoted.download() و m.download())
...
// تخمين mimetype من امتداد الملف
const MIME_MAP = {
...
function buildM(sock, chatId, message) {
    const m = {
        ...
        // m.msg = كائن المحتوى الخام (نمط GataBot: (m.quoted || m).msg.mimetype)
        msg: message.message || {},
        ...
        // تحميل الـ media من هذه الرسالة (صورة/فيديو/صوت)
        download: async () => {
            if (!_downloadMediaMessage) throw new Error('downloadMediaMessage غير متاح');
            return _downloadMediaMessage({ key: message.key, message: message.message }, sock);
        },
        ...
        if (ctxInfo?.quotedMessage) {
            ...
            m.quoted = {
                ...
                text: ctxInfo.quotedMessage.conversation ||
                      ...
                      '',
                message: ctxInfo.quotedMessage,
                // نمط GataBot: (m.quoted).msg.mimetype
                ...
                download: async () => {
                    if (!_downloadMediaMessage) throw new Error('downloadMediaMessage غير متاح');
                    return _downloadMediaMessage(quotedRaw, sock);
                },
            };
        }
    } catch (e) { /* ignore */ }
...
/**
 * بناء دالة sendFile متوافقة مع نمط GataBot:
 *   sendFile(jid, src, filename, caption, quoted, ptype, options)
 * src: URL string أو Buffer
 * ptype: 'document' أو null
 * options: { mimetype, asDocument, ... }
 */
function buildSendFile(sock) {
    return async function sendFile(jid, src, filename = '', caption = '', quoted = null, ptype = null, options = {}) {
        const asDocument = ptype === 'document' || options.asDocument || false;
        let mimetype = options.mimetype || '';
        if (!mimetype && typeof filename === 'string' && filename.includes('.')) {
            const ext = (filename.split('.').pop() || '').toLowerCase();
            mimetype = MIME_MAP[ext] || '';
        }
        const source = (src && typeof src === 'object') ? src : { url: String(src) };
        const extra = { ...options };
        delete extra.asDocument;
        ...
        if (mimetype.startsWith('video/')) {
            return sock.sendMessage(jid, { video: source, mimetype, caption: caption || undefined, ...extra }, quoted ? { quoted } : {});
        }
        if (mimetype.startsWith('audio/')) {
            return sock.sendMessage(jid, { audio: source, mimetype, ...extra }, quoted ? { quoted } : {});
        }
        if (mimetype.startsWith('image/')) {
            return sock.sendMessage(jid, { image: source, mimetype, caption: caption || undefined, ...extra }, quoted ? { quoted } : {});
        }
        // نوع غير معروف → أرسله كمستند (الأكثر أماناً)
        return sock.sendMessage(jid, {
            document: source,
            mimetype: mimetype || 'application/octet-stream',
            fileName: filename || 'file',
...
function buildConn(sock) {
    return {
        ...
        // رفع media للواتساب (مطلوب لـ prepareWAMessageMedia)
        waUploadToServer: sock.waUploadToServer || (async () => { throw new Error('waUploadToServer غير متاح'); }),

        // sendFile بنمط GataBot (مطلوب لإضافات التحميل)
        sendFile: buildSendFile(sock),
